...
describe("Test Oxor Precompile", function () {
	after(() => {
		web3.currentProvider.disconnect();
	});

	it("Deploy oxor contract", async function () {
	...
	it("Check after transfer and call 1", async function () {
		...
		// check apps, the caller oxor balance - 30
	}).timeout(80000);
	...
	it("Test Withdraw 10", async function () {
		...
		// check apps 2qSbd2umtD4KmV2X7kfttbP8HH4tzL5iMKETbjY2vYXMHHQs oxor balance = 10
	}).timeout(80000);
	...
	it("Test Withdraw 50", async function () {
		...
		// check apps 2qSbd2umtD4KmV2X7kfttbP8HH4tzL5iMKETbjY2vYXMHHQs oxor balance = 60
	}).timeout(80000);
});
