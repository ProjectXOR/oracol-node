...
fn test_evm_module() {
	ExtBuilder::default()
		.balances(vec![
			(alice_account_id(), CurrencyId::Token(TokenSymbol::XOR), amount(1 * MILLI_XOR)),
			(bob_account_id(), CurrencyId::Token(TokenSymbol::XOR), amount(1 * MILLI_XOR)),
		])
		.build()
		.execute_with(|| {
			assert_eq!(Balances::free_balance(alice_account_id()), amount(1 * MILLI_XOR));
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_XOR));
			...
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_XOR) - amount(10 * MICRO_XOR));
		});
}
