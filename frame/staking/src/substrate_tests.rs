...
fn force_unstake_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// Cant transfer
		assert_noop!(
			Xor::transfer(Origin::signed(11), 1, 10),
		...
		// Transfer works.
		assert_ok!(Xor::transfer(Origin::signed(11), 1, 10));
	});
}
...
fn basic_setup_works() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			...
			Exposure {
				own_xor_balance: 1000,
				own_oxor_balance: 0,
				own_power: Staking::power_of(&11),
				// Allow error rate 1
				total_power: Staking::power_of(&11) + xor_power(125) + 1,
				others: vec![IndividualExposure {
					who: 101,
					xor_balance: 125,
					oxor_balance: 0,
					// Allow error rate 1
					power: xor_power(125) + 1,
				}]
			}
		);
		...
		assert_eq!(
			...
			Exposure {
				...
				// Allow error rate 1
				total_power: Staking::power_of(&21) + xor_power(375) - 1,
				others: vec![IndividualExposure {
					who: 101,
					// Allow error rate 1
					xor_balance: 375 - 1,
		...
		assert_eq!(Xor::free_balance(10), 1);
		assert_eq!(Xor::free_balance(10), 1);
...
fn rewards_should_work() {
	ExtBuilder::default()
		...
		.sessions_per_era(3)
		.build_and_execute(|| {
			let init_balance_10 = Xor::free_balance(&10);
			let init_balance_11 = Xor::free_balance(&11);
			let init_balance_20 = Xor::free_balance(&20);
			let init_balance_21 = Xor::free_balance(&21);
			let init_balance_100 = Xor::free_balance(&100);
			let init_balance_101 = Xor::free_balance(&101);
			...
			assert_eq!(Xor::free_balance(&10), init_balance_10);
			assert_eq!(Xor::free_balance(&11), init_balance_11);
			assert_eq!(Xor::free_balance(&20), init_balance_20);
			assert_eq!(Xor::free_balance(&21), init_balance_21);
			assert_eq!(Xor::free_balance(&100), init_balance_100);
			assert_eq!(Xor::free_balance(&101), init_balance_101);
			...
			assert_eq!(
				XOR_REWARD_REMAINDER_UNBALANCED.with(|v| *v.borrow()),
			...
			assert_eq_error_rate!(Xor::free_balance(&11), init_balance_11, MICRO);
			assert_eq_error_rate!(
				Xor::free_balance(&20),
				init_balance_20 + part_for_20 * total_payout_0 * 1 / 3,
				MICRO,
			);
			assert_eq_error_rate!(Xor::free_balance(&21), init_balance_21, MICRO);
			assert_eq_error_rate!(
				Xor::free_balance(&100),
				init_balance_100
					+ part_for_100_from_10 * total_payout_0 * 2 / 3
					+ part_for_100_from_20 * total_payout_0 * 1 / 3,
				MICRO,
			);
			assert_eq_error_rate!(Xor::free_balance(&101), init_balance_101, MICRO);
...
fn staking_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 1..5 {
				let _ = Xor::make_free_balance_be(&i, 2000);
			}
			...
			assert_ok!(Staking::bond(
				Origin::signed(3),
				4,
				StakingBalance::XorBalance(1500),
			...
			assert_noop!(Xor::reserve(&3, 501), XorError::LiquidityRestrictions);
			assert_ok!(Xor::reserve(&3, 409));
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in [1, 2, 3, 4, 5, 10, 11, 20, 21].iter() {
				let _ = Xor::make_free_balance_be(i, initial_balance);
			}
			...
			assert_ok!(Staking::bond(
				Origin::signed(1),
				2,
				StakingBalance::XorBalance(1000),
				RewardDestination::Controller,
				0,
			));
			...
			make_all_reward_payment(0);
			assert_eq!(Xor::free_balance(&40), 1 + total_payout_0 / 2);
			assert_eq!(Xor::free_balance(&30), 1 + total_payout_0 / 2);
			...
			assert_eq!(
				...
				Exposure {
					own_xor_balance: 1000,
					own_oxor_balance: 0,
					own_power: xor_power(1000),
					total_power: xor_power(1000) + xor_power(800),
					others: vec![
						IndividualExposure {
							who: 3,
							xor_balance: 400,
			...
			assert_eq!(
				...
				Exposure {
					...
					own_oxor_balance: 0,
					own_power: xor_power(1000),
					total_power: xor_power(1000) + xor_power(1200),
					others: vec![
						IndividualExposure {
							who: 3,
...
fn nominators_also_get_slashed_pro_rata() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// staked values;
		let nominator_stake = Staking::ledger(100).unwrap().active_xor;
		let nominator_balance = xor_balances(&101).0;
		let validator_stake = Staking::ledger(10).unwrap().active_xor;
		let validator_balance = xor_balances(&11).0;
		...
		assert!(Staking::ledger(100).unwrap().active_xor < nominator_stake);
		assert!(Staking::ledger(10).unwrap().active_xor < validator_stake);
		...
		assert_eq_error_rate!(
			Staking::ledger(100).unwrap().active_xor,
			nominator_stake - nominator_share,
			10,
		);
		assert_eq_error_rate!(
			Staking::ledger(10).unwrap().active_xor,
			validator_stake - validator_share,
			10,
		);
		assert_eq_error_rate!(
			xor_balances(&101).0, // free balance
			nominator_balance - nominator_share,
			10,
		);
		assert_eq_error_rate!(
			xor_balances(&11).0, // free balance
...
fn double_staking_should_fail() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::bond(
			Origin::signed(1),
			2,
			StakingBalance::XorBalance(arbitrary_value),
...
fn cannot_transfer_staked_balance() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(11), 1000);
		// Confirm account 11 (via controller 10) is totally staked
		{
			...
			assert_eq!(
				expo.others
					.iter()
					.fold(expo.own_xor_balance, |acc, i| acc + i.xor_balance),
				1000,
			);
		}
		// Confirm account 11 cannot transfer as a result
		assert_noop!(
			Xor::transfer(Origin::signed(11), 20, 1),
			XorError::LiquidityRestrictions,
		);

		// Give account 11 extra free balance
		let _ = Xor::make_free_balance_be(&11, 10000);
		// Confirm that account 11 can now transfer some balance
		assert_ok!(Xor::transfer(Origin::signed(11), 20, 1));
	});
}
...
fn cannot_transfer_staked_balance_2() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Xor::free_balance(21), 2000);
			...
			assert_noop!(
				Xor::transfer(Origin::signed(21), 20, 1001),
				XorError::LiquidityRestrictions,
			);
			assert_ok!(Xor::transfer(Origin::signed(21), 20, 1000));
		});
}
...
fn cannot_reserve_staked_balance() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		// Confirm account 11 (via controller 10) is totally staked
		assert_eq!(
			Staking::eras_stakers(active_era(), &11).own_xor_balance,
			1000,
		);
		// Confirm account 11 cannot transfer as a result
		assert_noop!(Xor::reserve(&11, 1), XorError::LiquidityRestrictions);

		// Give account 11 extra free balance
		let _ = Xor::make_free_balance_be(&11, 10000);
		// Confirm account 11 can now reserve balance
		assert_ok!(Xor::reserve(&11, 1));
	});
}
...
fn reward_destination_works() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(11), 1000 + total_payout_0);
		// Check that amount at stake increased accordingly
		assert_eq!(
			Staking::ledger(&10),
			Some(StakingLedger {
				stash: 11,
				active_xor: 1000 + total_payout_0,
				xor_staking_lock: StakingLock {
		...
		assert_eq!(Xor::free_balance(10), 1 + total_payout_2);
		// Check that amount at stake is NOT increased
		assert_eq!(
			Staking::ledger(&10),
			Some(StakingLedger {
		...
		assert_eq!(Xor::free_balance(11), recorded_stash_balance);
	});
}
...
fn validator_payment_prefs_work() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		let balance_era_1_10 = Xor::free_balance(&10);
		let balance_era_1_100 = Xor::free_balance(&100);
...
fn bond_extra_works() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(Staking::bond_extra(
			Origin::signed(11),
			StakingBalance::XorBalance(Balance::max_value()),
			0,
		));
...
fn too_many_unbond_calls_should_not_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// can't do more.
		assert_noop!(
			Staking::unbond(Origin::signed(10), StakingBalance::XorBalance(1)),
...
fn rebond_works() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			Staking::unbond(Origin::signed(10), StakingBalance::XorBalance(900)).unwrap();
			assert_eq!(
				Staking::ledger(&10),
				Some(StakingLedger {
					stash: 11,
			...
			Staking::unbond(Origin::signed(10), StakingBalance::XorBalance(300)).unwrap();
			Staking::unbond(Origin::signed(10), StakingBalance::XorBalance(300)).unwrap();
			Staking::unbond(Origin::signed(10), StakingBalance::XorBalance(300)).unwrap();
			assert_eq!(
				Staking::ledger(&10),
...
fn rebond_is_fifo() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			Staking::unbond(Origin::signed(10), StakingBalance::XorBalance(200)).unwrap();
			assert_eq!(
				Staking::ledger(&10),
				Some(StakingLedger {
					stash: 11,
		...
		.build_and_execute(|| {
			...
			}
			{
				...
				let total_xor = expo
					...
					.iter()
					.fold(expo.own_xor_balance, |acc, i| acc + i.xor_balance);
				assert_eq!(total_xor, 2000);
			}

			// Give the man some money.
			let _ = Xor::make_free_balance_be(&10, 1000);
			let _ = Xor::make_free_balance_be(&20, 1000);
			...
			<ErasStakers<Test>>::insert(
				...
				Exposure {
					own_xor_balance: 69,
					own_oxor_balance: 0,
					total_power: xor_power(69),
					own_power: xor_power(69),
					others: vec![],
				},
			);
			...
			let _11_balance = Xor::free_balance(&11);
			...
			// -- new infos
			{
				...
				let total_xor = expo
					...
					.iter()
					.fold(expo.own_xor_balance, |acc, i| acc + i.xor_balance);
				assert_eq!(total_xor, 1000 + total_payout_0 / 2);
			}
			{
				let expo = Staking::eras_stakers(active_era(), 21);
				let total_xor = expo
					...
					.iter()
					.fold(expo.own_xor_balance, |acc, i| acc + i.xor_balance);
				assert_eq!(total_xor, 69 + total_payout_0 / 2);
			}
		});
}
...
fn on_free_balance_zero_stash_removes_validator() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			// Check the balance of the validator account
			assert_eq!(Xor::free_balance(10), 256);
			// Check the balance of the stash account
			assert_eq!(Xor::free_balance(11), 256000);
			...
			let _ = Xor::slash(&10, Balance::max_value());
			...
			let _ = Xor::slash(&11, Balance::max_value());
			// Check total balance of stash
			assert_eq!(Xor::free_balance(&11), 10);
...
fn on_free_balance_zero_stash_removes_nominator() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Xor::free_balance(&10), 0);
...
fn switching_roles() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		// put some money in account that we'll use.
		for i in 1..7 {
			let _ = Xor::deposit_creating(&i, 5000);
		}
		...
		assert_ok!(Staking::bond(
			Origin::signed(1),
			2,
			StakingBalance::XorBalance(2000),
			RewardDestination::Controller,
			0,
		));
...
fn bond_with_no_staked_value() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Xor::locks(&1)[0].locked_amount(Some(System::block_number())),
				5
			);
...
fn bond_with_little_staked_value_bounded() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let init_balance_2 = Xor::free_balance(&2);
			let init_balance_10 = Xor::free_balance(&10);
			...
			assert_eq!(Xor::free_balance(&2), init_balance_2);
			...
			assert_eq!(
				...
				vec![
					(
						...
						Support {
							// error rate 1
							total: (xor_power(1800) + 1) as _,
							voters: vec![
								(21, xor_power(1000) as _),
								(3, xor_power(400) as _),
								(1, xor_power(400) as _)
							]
						}
					),
					(
						31,
						Support {
							total: xor_power(2200) as _,
							voters: vec![
								(31, xor_power(1000) as _),
								(3, xor_power(600) as _),
								(1, xor_power(600) as _)
							]
						}
					)
				],
...
fn bond_with_duplicate_vote_should_be_ignored_by_election_provider_elected() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Staking::bond_extra(
				Origin::signed(31),
				StakingBalance::XorBalance(99),
				0
			));
			// ensure all have equal stake.
			assert_eq!(
				<Validators<Test>>::iter()
					.map(|(v, _)| (v, Staking::ledger(v - 1).unwrap().active_xor))
					.collect::<Vec<_>>(),
			...
			assert_eq!(
				...
				vec![
					(
						11,
						Support {
							total: xor_power(1500) as _,
							voters: vec![(11, xor_power(1000) as _), (1, xor_power(500) as _)]
						}
					),
					(
						21,
						Support {
							// error rate 1
							total: (xor_power(2500) - 1) as _,
							voters: vec![
								(21, xor_power(1000) as _),
								(3, xor_power(1000) as _),
								(1, xor_power(500) as _)
							]
						}
					)
				],
...
fn phragmen_should_not_overflow() {
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		bond_validator(3, 2, StakingBalance::XorBalance(CAP));
		bond_validator(5, 4, StakingBalance::OxorBalance(Balance::max_value() - 1));

		bond_nominator(7, 6, StakingBalance::XorBalance(1), vec![3, 5]);
		bond_nominator(9, 8, StakingBalance::OxorBalance(1), vec![3, 5]);
...
fn reward_validator_slashing_validator_does_not_overflow() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// Set staker
		let _ = Xor::make_free_balance_be(&11, stake);

		let exposure = Exposure::<AccountId, Balance, Balance> {
			own_xor_balance: stake,
			own_oxor_balance: 0,
			own_power: xor_power(stake),
			total_power: xor_power(stake),
			others: vec![],
		};
		...
		let _ = Xor::deposit_creating(&Staking::account_id(), stake);
		assert_ok!(Staking::payout_stakers(Origin::signed(1337), 11, 0));
		assert_eq!(Xor::free_balance(&11), stake * 2);

		// Set staker
		let _ = Xor::make_free_balance_be(&11, stake);
		let _ = Xor::make_free_balance_be(&2, stake);
		...
		Staking::bond(
			Origin::signed(2),
			20000,
			StakingBalance::XorBalance(stake - 1),
			RewardDestination::default(),
			0,
		)
		.unwrap();
		...
		<ErasStakers<Test>>::insert(
			...
			Exposure {
				...
				total_power: xor_power(stake),
				own_oxor_balance: 0,
				own_power: xor_power(1),
				others: vec![IndividualExposure {
					who: 2,
					xor_balance: stake - 1,
					oxor_balance: 0,
					power: xor_power(stake - 1),
				}],
			},
		);
		...
		assert_eq_error_rate!(Xor::free_balance(&11), stake - 1000, MICRO);
		assert_eq!(Xor::free_balance(&2), 1);
	})
}
...
fn slashing_performed_according_exposure() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(11), 1000 - 250);
	});
}
	...
	ExtBuilder::default().build_and_execute(|| {
...

		{
			...
			let total_xor = expo
				...
				.iter()
				.fold(expo.own_xor_balance, |acc, i| acc + i.xor_balance);
			assert_eq!(total_xor, initial_balance);
		}
		...
		assert_eq!(Xor::free_balance(&1), 10 + reward_each);
		assert_eq!(Xor::free_balance(&2), 20 + reward_each);
	});
}
...
fn subsequent_reports_in_same_span_pay_out_less() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(&1), 10 + reward);
		...
		assert_eq!(Xor::free_balance(&1), 10 + prior_payout + reward);
	});
}
...
fn only_slash_for_max_in_era() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(11), 500);
		assert_eq!(Staking::force_era(), Forcing::ForceNew);
		...
		assert_eq!(Xor::free_balance(11), 400);
	})
}
...
fn garbage_collection_after_slashing() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			assert_eq!(Xor::free_balance(11), 256_000);
			...
			assert_eq!(Xor::free_balance(11), 256_000 - 25_600);
			...
			assert_eq!(Xor::free_balance(11), 2);
			assert_eq!(Xor::total_balance(&11), 2);
...
fn garbage_collection_on_window_pruning() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(101), 2000);
		...
		assert_eq!(Xor::free_balance(11), 900);
		assert_eq!(Xor::free_balance(101), 2000 - (nominated_value / 10));
...
fn slashing_nominators_by_span_max() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(101), 2000 - slash_1_amount);
		...
		assert_eq!(Xor::free_balance(11), 900);
		assert_eq!(Xor::free_balance(21), 1700);

		let slash_2_amount = Perbill::from_percent(30) * nominated_value_21;
		assert!(slash_2_amount > slash_1_amount);

		// only the maximum slash in a single span is taken.
		assert_eq!(Xor::free_balance(101), 2000 - slash_2_amount);
		...
		assert_eq!(Xor::free_balance(11), 800);
		assert_eq!(Xor::free_balance(21), 1700);
...
fn slashes_are_summed_across_spans() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Xor::free_balance(21), 1900);
		...
		assert_eq!(get_span(21).iter().collect::<Vec<_>>(), expected_spans);
		assert_eq!(Xor::free_balance(21), 1810);
	});
}
...
fn remove_deferred() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Xor::free_balance(11), 950);
			assert_eq!(Xor::free_balance(101), 2000 - actual_slash);
		})
}
...
fn slash_kicks_validators_not_nominators_and_disables_nominator_for_kicked_validator() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq_error_rate!(exposure_11.total_power, xor_power(1000 + 125), 1);
		assert_eq_error_rate!(exposure_21.total_power, xor_power(1000 + 375), 1);
		...
		assert_eq!(Xor::free_balance(11), 900);
		assert_eq!(Xor::free_balance(101), 2000 - nominator_slash_amount_11);
		...
		assert_eq!(exposure_11.total_power, xor_power(900));

		// 20 is re-elected, with the (almost) entire support of 100
		assert_eq_error_rate!(
			exposure_21.total_power,
			xor_power(1000 + 500 - nominator_slash_amount_11),
			10
		);
	});
}
...
fn claim_reward_at_the_last_era_and_no_double_claim_and_invalid_claim() {
	...
	ExtBuilder::default().nominate(true).build_and_execute(|| {
		...
		let _ = Xor::deposit_creating(&999, 1_000_000_000);
		// Compute total payout now for whole duration as other parameter won't change
		let total_payout_1 = current_total_payout_for_duration(reward_time_per_era());
...
fn test_max_nominator_rewarded_per_validator_and_cant_steal_someone_else_reward() {
	ExtBuilder::default().build_and_execute(|| {
		for i in 0..=<Test as Config>::MaxNominatorRewardedPerValidator::get() {
			...
			Xor::make_free_balance_be(&stash, balance);
			assert_ok!(Staking::bond(
				Origin::signed(stash),
				controller,
				StakingBalance::XorBalance(balance),
				RewardDestination::Stash,
				0,
			));
		...
		for i in 0..=<Test as Config>::MaxNominatorRewardedPerValidator::get() {
			...
			let balance = 10_000 + i as Balance;
			if stash == 10_000 {
				assert!(Xor::free_balance(&stash) == balance);
			} else {
				assert!(Xor::free_balance(&stash) > balance);
			}
		}
...
fn test_payout_stakers() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			let balance = 1000;
			// Create a validators:
			bond_validator(11, 10, StakingBalance::XorBalance(balance)); // Default(64)
			...
			for i in 0..100 {
				bond_nominator(
					1000 + i,
					100 + i,
					StakingBalance::XorBalance(balance + i as Balance),
					vec![11],
				);
			}
			...
			assert!(Xor::free_balance(&10) > balance);
			for i in 36..100 {
				assert!(Xor::free_balance(&(100 + i)) > balance + i as Balance);
			}
			// The bottom 36 do not
			for i in 0..36 {
				assert_eq!(Xor::free_balance(&(100 + i)), balance + i as Balance);
			}
...
#[test]
fn bond_duxor_era_correctly_populates_claimed_rewards() {
	ExtBuilder::default()
		.has_stakers(false)
		.build_and_execute(|| {
			// Era = None
			bond_validator(9, 8, StakingBalance::XorBalance(1000));
			assert_eq!(
				Staking::ledger(&8),
				Some(StakingLedger {
					stash: 9,
			...
			start_active_era(99);
			bond_validator(13, 12, StakingBalance::XorBalance(1000));
			assert_eq!(
				Staking::ledger(&12),
				Some(StakingLedger {
					stash: 13,
...
fn payout_creates_controller() {
	ExtBuilder::default()
		.has_stakers(false)
		.build_and_execute(|| {
			let balance = StakingBalance::XorBalance(1000);
			// Create a validator:
			bond_validator(11, 10, balance);

			// Create a stash/controller pair
			bond_nominator(1234, 1337, StakingBalance::XorBalance(100), vec![11]);

			// kill controller
			assert_ok!(Xor::transfer(Origin::signed(1337), 1234, 100));
			assert_eq!(Xor::free_balance(1337), 0);
			...
			assert!(Xor::free_balance(1337) > 0);
		})
}
...
fn payout_to_any_account_works() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Xor::free_balance(42), 0);
			...
			assert!(Xor::free_balance(42) > 0);
		})
}

#[test]
fn session_buffexor_with_offset() {
...
fn cannot_rebond_to_lower_than_ed() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Xor::free_balance(&21), 512_000);
