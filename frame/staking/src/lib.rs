...
//! <!-- Original author of paragraph: @gavofyork -->
//!
//! The staking system in Oracol NPoS is designed to make the following possible:
//!
...
mod types {
	...
	pub type XorPositiveImbalance<T> =
		<XorCurrency<T> as Currency<AccountId<T>>>::PositiveImbalance;
	pub type XorNegativeImbalance<T> =
		<XorCurrency<T> as Currency<AccountId<T>>>::NegativeImbalance;

	/// The balance type of this module.
	pub type OxorBalance<T> = <OxorCurrency<T> as Currency<AccountId<T>>>::Balance;
	pub type OxorPositiveImbalance<T> =
		<OxorCurrency<T> as Currency<AccountId<T>>>::PositiveImbalance;
	pub type OxorNegativeImbalance<T> =
		<OxorCurrency<T> as Currency<AccountId<T>>>::NegativeImbalance;

	pub type StakingLedgerT<T> =
		StakingLedger<AccountId<T>, XorBalance<T>, OxorBalance<T>, BlockNumber<T>>;
	pub type StakingBalanceT<T> = StakingBalance<XorBalance<T>, OxorBalance<T>>;

	pub type ExposureT<T> = Exposure<AccountId<T>, XorBalance<T>, OxorBalance<T>>;
...
// --- oracol ---
use oracol_staking_rpc_runtime_api::RuntimeDispatchInfo;
use oracol_support::{
	balance::*,
	impl_rpc,
	traits::{OnDepositRedeem, OnUnbalancedOxor},
};
use types::*;
...
pub trait Config: frame_system::Config + SendTransactionTypes<Call<Self>> {
	...
	type XorRewardRemainder: OnUnbalanced<XorNegativeImbalance<Self>>;
	/// Handler for the unbalanced *XOR* reduction when slashing a staker.
	type XorSlash: OnUnbalanced<XorNegativeImbalance<Self>>;
	/// Handler for the unbalanced *XOR* increment when rewarding a staker.
	type XorReward: OnUnbalanced<XorPositiveImbalance<Self>>;
	...
	/// Handler for the unbalanced *OXOR* reduction when slashing a staker.
	type OxorSlash: OnUnbalancedOxor<OxorNegativeImbalance<Self>>;
	/// Handler for the unbalanced *OXOR* increment when rewarding a staker.
	type OxorReward: OnUnbalanced<OxorPositiveImbalance<Self>>;

	/// Oracol's hard cap default 10_000_000_000 * 10^9
	type Cap: Get<XorBalance<Self>>;
	/// Oracol's staking vote default 1_000_000_000
...
where
	...
	T: pallet_session::historical::Config<
		FullIdentification = Exposure<AccountId<T>, XorBalance<T>, OxorBalance<T>>,
...
decl_event!(
		...
		OxorBalance = OxorBalance<T>,
	{
		...
		EraPayout(EraIndex, XorBalance, XorBalance),

		/// The staker has been rewarded by this amount. [stash, amount]
		Reward(AccountId, XorBalance),

		/// One validator (and its nominators) has been slashed by the given amount.
		/// [validator, amount, amount]
		Slash(AccountId, XorBalance, OxorBalance),
		...
		BondXor(XorBalance, TsInMs, TsInMs),
		/// An account has bonded this amount. [amount, start, end]
		///
		/// NOTE: This event is only emitted when funds are bonded via a dispatchable. Notably,
		/// it will not be emitted for staking rewards when they are added to stake.
		BondOxor(OxorBalance),

		/// An account has unbonded this amount. [amount, now]
		UnbondXor(XorBalance, BlockNumber),
		/// An account has unbonded this amount. [amount, now]
		UnbondOxor(OxorBalance, BlockNumber),
		...
		/// Someone claimed his deposits with some *OXOR*s punishment. [stash, forfeit]
		DepositsClaimedWithPunish(AccountId, OxorBalance),
	}
);
...
decl_storage! {
	trait Store for Module<T: Config> as OracolStaking {
		...
		pub ErasValidatorReward
			...
			: map hasher(twox_64_concat) EraIndex => Option<XorBalance<T>>;
		...
		pub UnappliedSlashes
			...
			=> Vec<UnappliedSlash<T::AccountId, XorBalance<T>, OxorBalance<T>>>;
		...
		SpanSlash
			...
			=> slashing::SpanRecord<XorBalance<T>, OxorBalance<T>>;
		...
		/// use for calculate oracol era payout
		...
		/// Total *XOR* in pool.
		pub XorPool get(fn xor_pool): XorBalance<T>;
		/// Total *OXOR* in pool.
		pub OxorPool get(fn oxor_pool): OxorBalance<T>;
	}
	add_extra_genesis {
		config(stakers): Vec<(
			T::AccountId,
		...
		build(|config: &GenesisConfig<T>| {
			for &(ref stash, ref controller, xor_to_be_bonded, ref status) in &config.stakers {
				assert!(
					T::XorCurrency::free_balance(&stash) >= xor_to_be_bonded,
					"Stash does not have enough balance to bond.",
				);
				let _ = <Module<T>>::bond(
					T::Origin::from(Some(stash.to_owned()).into()),
					T::Lookup::unlookup(controller.to_owned()),
					StakingBalance::XorBalance(xor_to_be_bonded),
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		/// Oracol's hard cap default 10_000_000_000 * 10^9
		const Cap: XorBalance<T> = T::Cap::get();
		...
		fn bond(
			...
			match value {
				StakingBalance::XorBalance(value) => {
					// reject a bond which is considered to be _dust_.
						...
						value >= T::XorCurrency::minimum_balance(),
						<Error<T>>::InsufficientValue,
					);
				}
				StakingBalance::OxorBalance(value) => {
					// reject a bond which is considered to be _dust_.
					ensure!(
						value >= T::OxorCurrency::minimum_balance(),
						<Error<T>>::InsufficientValue,
					);
				}
			}
			...
			match value {
				StakingBalance::XorBalance(value) => {
					let stash_balance = T::XorCurrency::free_balance(&stash);
					let value = value.min(stash_balance);
					let promise_month = promise_month.min(36);
					let (start_time, expire_time) = Self::bond_xor(
						&stash,
						&controller,
						value,
						promise_month,
					...
					Self::deposit_event(RawEvent::BondXor(value, start_time, expire_time));
				},
				StakingBalance::OxorBalance(value) => {
					let stash_balance = T::OxorCurrency::free_balance(&stash);
					let value = value.min(stash_balance);

					Self::bond_oxor(&controller, value, ledger)?;
					Self::deposit_event(RawEvent::BondOxor(value));
				},
			}
		}
		...
		fn bond_extra(origin, max_additional: StakingBalanceT<T>, promise_month: u8) {
			...
			match max_additional {
				 StakingBalance::XorBalance(max_additional) => {
					let stash_balance = T::XorCurrency::free_balance(&stash);

					if let Some(extra) = stash_balance.checked_sub(
						&ledger.xor_locked_amount_at(<frame_system::Pallet<T>>::block_number())
					) {
						...
						Self::deposit_event(RawEvent::BondXor(extra, start_time, expire_time));
					}
				},
				StakingBalance::OxorBalance(max_additional) => {
					let stash_balance = T::OxorCurrency::free_balance(&stash);

					if let Some(extra) = stash_balance.checked_sub(
						&ledger.oxor_locked_amount_at(<frame_system::Pallet<T>>::block_number())
					) {
						let extra = extra.min(max_additional);

						Self::bond_oxor(&controller, extra, ledger)?;
						Self::deposit_event(RawEvent::BondOxor(extra));
					}
				},
			}
		}

		/// Deposit some extra amount ring, and return oxor to the controller.
		///
		...
		fn deposit_extra(origin, value: XorBalance<T>, promise_month: u8) {
			let stash = ensure_signed(origin)?;
			let controller = Self::bonded(&stash).ok_or(<Error<T>>::NotStash)?;
			let ledger = Self::ledger(&controller).ok_or(<Error<T>>::NotController)?;
			...
			let value = value.min(active_xor.saturating_sub(*active_deposit_xor));

			if value.is_zero() {
				return Ok(());
			}

			let oxor_return = inflation::compute_oxor_reward::<T>(value, promise_month);
			let oxor_positive_imbalance = T::OxorCurrency::deposit_creating(&stash, oxor_return);

			T::OxorReward::on_unbalanced(oxor_positive_imbalance);
			*active_deposit_xor = active_deposit_xor.saturating_add(value);
		...
		///
		/// After all pledged Xor and Oxor are unbonded, the bonded accounts, namely stash and
		/// controller, will also be unbonded.  Once user want to bond again, the `bond` method
		/// should be called. If there are still pledged Xor or Oxor and user want to bond more
		...
		fn unbond(origin, value: StakingBalanceT<T>) {
			...
			xor_staking_lock.update(now);
			oxor_staking_lock.update(now);
			...
			//	 2. `let c_ = a as u32 + b as u32; c_ < c`
				...
				(xor_staking_lock.unbondings.len() + oxor_staking_lock.unbondings.len())
					< MAX_UNLOCKING_CHUNKS,
				<Error<T>>::NoMoreChunks,
			);

			let mut unbond_xor: XorBalance<T> = Zero::zero();
			let mut unbond_oxor: OxorBalance<T> = Zero::zero();

			match value {
				StakingBalance::XorBalance(r) => {
					// Only active normal ring can be unbond:
					// `active_xor = active_normal_xor + active_deposit_xor`
					let active_normal_xor = *active_xor - *active_deposit_xor;
					unbond_xor = r.min(active_normal_xor);

					if !unbond_xor.is_zero() {
						*active_xor -= unbond_xor;

						// Avoid there being a dust balance left in the staking system.
						if (*active_xor < T::XorCurrency::minimum_balance())
							&& (*active_oxor < T::OxorCurrency::minimum_balance()) {
							unbond_xor += *active_xor;
							unbond_oxor += *active_oxor;

							*active_xor = Zero::zero();
							*active_oxor = Zero::zero();
						}

						xor_staking_lock.unbondings.push(Unbonding {
							amount: unbond_xor,
							until: now + T::BondingDurationInBlockNumber::get(),
						});

						Self::deposit_event(RawEvent::UnbondXor(unbond_xor, now));

						if !unbond_oxor.is_zero() {
							oxor_staking_lock.unbondings.push(Unbonding {
								amount: unbond_oxor,
								until: now + T::BondingDurationInBlockNumber::get(),
							});

							Self::deposit_event(RawEvent::UnbondOxor(unbond_oxor, now));
						}
					}
				},
				StakingBalance::OxorBalance(k) => {
					unbond_oxor = k.min(*active_oxor);

					if !unbond_oxor.is_zero() {
						*active_oxor -= unbond_oxor;

						// Avoid there being a dust balance left in the staking system.
						if (*active_oxor < T::OxorCurrency::minimum_balance())
							&& (*active_xor < T::XorCurrency::minimum_balance()) {
							unbond_oxor += *active_oxor;
							unbond_xor += *active_xor;
			...
			// TODO: https://github.com/ProjectXOR/oracol-nodeissues/96
			// FIXME: https://github.com/ProjectXOR/oracol-nodeissues/121
			// let StakingLedger {
			// 	active_xor,
			// 	active_oxor,
			// 	..
			...
			// // All bonded *XOR* and *OXOR* is withdrawing, then remove Ledger to save storage
			// if active_xor.is_zero() && active_oxor.is_zero() {
			// 	//
			...
			// 	// blocks.
			// 	//
			// 	// T::Xor::remove_lock(STAKING_ID, &stash);
			// 	// T::Oxor::remove_lock(STAKING_ID, &stash);
		...
		/// will not be slashed, but the account is required to pay OXOR as punish.
		///
		/// Refer to https://talk.oracol.network/topics/55
		///
...

			{
				...
				deposit_items.retain(|item| {
					...
					let oxor_slash = {
						...
						(
							inflation::compute_oxor_reward::<T>(
					...
					// strict on punishing in oxor
					if T::OxorCurrency::usable_balance(stash) >= oxor_slash {
						*active_deposit_xor = active_deposit_xor.saturating_sub(item.value);

						let imbalance = T::OxorCurrency::slash(stash, oxor_slash).0;
						T::OxorSlash::on_unbalanced(imbalance);

						claim_deposits_with_punish = (true, oxor_slash);
		...
		fn force_unstake(origin, stash: T::AccountId, num_slashing_spans: u32) {
			...
			// remove the lock.
			T::XorCurrency::remove_lock(STAKING_ID, &stash);
			T::OxorCurrency::remove_lock(STAKING_ID, &stash);
		}
		...
		fn rebond(
			origin,
			#[compact] plan_to_rebond_xor: XorBalance<T>,
			#[compact] plan_to_rebond_oxor: OxorBalance<T>
		) -> DispatchResultWithPostInfo {
			let controller = ensure_signed(origin)?;
			let mut ledger = Self::ledger(&controller).ok_or(<Error<T>>::NotController)?;
			let now = <frame_system::Pallet<T>>::block_number();

			ledger.xor_staking_lock.update(now);
			ledger.oxor_staking_lock.update(now);

			ensure!(
				!ledger.xor_staking_lock.unbondings.is_empty()
					|| !ledger.oxor_staking_lock.unbondings.is_empty(),
				<Error<T>>::NoUnlockChunk
			);

			let origin_active_xor = ledger.active_xor;
			let origin_active_oxor = ledger.active_oxor;

			ledger.rebond(plan_to_rebond_xor, plan_to_rebond_oxor);

			// last check: the new active amount of ledger must be more than ED.
			ensure!(
				ledger.active_xor >= T::XorCurrency::minimum_balance()
					|| ledger.active_oxor >= T::OxorCurrency::minimum_balance(),
				<Error<T>>::InsufficientValue
			);

			Self::update_ledger(&controller, &mut ledger);

			let rebond_xor = ledger.active_xor.saturating_sub(origin_active_xor);
			let rebond_oxor = ledger.active_oxor.saturating_sub(origin_active_oxor);

			if !rebond_xor.is_zero() {
				let now = T::UnixTime::now().as_millis().saturated_into::<TsInMs>();

				Self::deposit_event(RawEvent::BondXor(rebond_xor, now, now));
			}
			if !rebond_oxor.is_zero() {
				Self::deposit_event(RawEvent::BondOxor(rebond_oxor));
			}

			Ok(Some(
				35 * WEIGHT_PER_MICROS
					+ 50 * WEIGHT_PER_NANOS * (
						ledger.xor_staking_lock.unbondings.len() as Weight
							+ ledger.oxor_staking_lock.unbondings.len() as Weight
					)
		...
		fn reap_stash(_origin, stash: T::AccountId, num_slashing_spans: u32) {
			let total_xor = T::XorCurrency::total_balance(&stash);
			let minimum_xor = T::XorCurrency::minimum_balance();
			let total_oxor = T::OxorCurrency::total_balance(&stash);
			let minimum_oxor = T::OxorCurrency::minimum_balance();
			let at_minimum = (
				total_xor == minimum_xor
				&&
				total_oxor <= minimum_oxor
			) || (
				total_oxor == minimum_oxor
				&&
				total_xor <= minimum_xor
...
impl<T: Config> Module<T> {
	...
	/// Update the ledger while bonding ring and compute the *OXOR* reward
	fn bond_xor(
		stash: &T::AccountId,
		...
		*active_xor = active_xor.saturating_add(value);
		...
		// there will be extra reward (*OXOR*), which can also be used for staking
		if promise_month > 0 {
			expire_time += promise_month as TsInMs * MONTH_IN_MILLISECONDS;
			*active_deposit_xor += value;
	...
	/// Update the ledger while bonding controller with *OXOR*
	fn bond_oxor(
		controller: &T::AccountId,
		value: OxorBalance<T>,
		mut ledger: StakingLedgerT<T>,
	) -> DispatchResult {
		ledger.active_oxor = ledger.active_oxor.saturating_add(value);
	...
	// For *XOR* power = xor_ratio * POWER_COUNT / 2
	// For *OXOR* power = oxor_ratio * POWER_COUNT / 2
	...
	pub fn power_of(stash: &T::AccountId) -> Power {
		...
		Self::bonded(stash)
			...
			.map(|l| {
				Self::currency_to_power::<_>(l.active_xor, Self::xor_pool())
					+ Self::currency_to_power::<_>(l.active_oxor, Self::oxor_pool())
			})
			.unwrap_or_default()
	}
	...
	pub fn stake_of(stash: &T::AccountId) -> (XorBalance<T>, OxorBalance<T>) {
		// Weight note: consider making the stake accessible through stash.
		Self::bonded(stash)
			.and_then(Self::ledger)
			.map(|l| (l.active_xor, l.active_oxor))
			.unwrap_or_default()
	}
	...
	fn do_payout_stakers(validator_stash: T::AccountId, era: EraIndex) -> DispatchResult {
		...
		let module_account = Self::account_id();
			...
			T::XorCurrency::usable_balance(&module_account) >= validator_total_payout,
			<Error<T>>::PayoutIns
		);
		...
		let mut actual_payout = <XorPositiveImbalance<T>>::zero();
		...
		for nominator in exposure.others.iter() {
			...
			let nominator_reward: XorBalance<T> =
	...
	fn update_ledger(controller: &T::AccountId, ledger: &mut StakingLedgerT<T>) {
		...
		if *active_xor != xor_staking_lock.staking_amount {
			let origin_active_xor = xor_staking_lock.staking_amount;

			xor_staking_lock.staking_amount = *active_xor;

			<XorPool<T>>::mutate(|pool| {
				if origin_active_xor > *active_xor {
					*pool = pool.saturating_sub(origin_active_xor - *active_xor);
				} else {
					*pool = pool.saturating_add(*active_xor - origin_active_xor);
				}
			});

			T::XorCurrency::set_lock(
				STAKING_ID,
				&ledger.stash,
				LockFor::Staking(ledger.xor_staking_lock.clone()),
				WithdrawReasons::all(),
			);
		}

		if *active_oxor != oxor_staking_lock.staking_amount {
			let origin_active_oxor = oxor_staking_lock.staking_amount;

			oxor_staking_lock.staking_amount = *active_oxor;

			<OxorPool<T>>::mutate(|pool| {
				if origin_active_oxor > *active_oxor {
					*pool = pool.saturating_sub(origin_active_oxor - *active_oxor);
				} else {
					*pool = pool.saturating_add(*active_oxor - origin_active_oxor);
				}
			});

			T::OxorCurrency::set_lock(
				STAKING_ID,
				&ledger.stash,
				LockFor::Staking(ledger.oxor_staking_lock.clone()),
				WithdrawReasons::all(),
			);
		}
	...
	fn make_payout(
		stash: &T::AccountId,
		amount: XorBalance<T>,
	) -> Option<XorPositiveImbalance<T>> {
		let dest = Self::payee(stash);
		match dest {
			RewardDestination::Controller => Self::bonded(stash).and_then(|controller| {
				Some(T::XorCurrency::deposit_creating(&controller, amount))
			}),
			RewardDestination::Stash => T::XorCurrency::deposit_into_existing(stash, amount).ok(),
			RewardDestination::Staked => Self::bonded(stash)
				.and_then(|c| Self::ledger(&c).map(|l| (c, l)))
				.and_then(|(c, mut l)| {
					let r = T::XorCurrency::deposit_into_existing(stash, amount).ok();
			...
			RewardDestination::Account(dest_account) => {
				Some(T::XorCurrency::deposit_creating(&dest_account, amount))
			}
	...
	fn end_era(active_era: ActiveEraInfo, _session_index: SessionIndex) {
		...
		if let Some(active_era_start) = active_era.start {
			...
			let (validator_payout, max_payout) = inflation::compute_total_payout::<T>(
				era_duration,
				Self::living_time(),
				T::Cap::get().saturating_sub(T::XorCurrency::total_issuance()),
				PayoutFraction::get(),
			);
			...
			T::XorCurrency::deposit_creating(&Self::account_id(), validator_payout);
			T::XorRewardRemainder::on_unbalanced(T::XorCurrency::issue(rest));
		}
	}
	...
	fn collect_exposures(supports: Supports<T::AccountId>) -> Vec<(T::AccountId, ExposureT<T>)> {
		supports
			...
			.map(|(validator, support)| {
				...
				let mut own_xor_balance: XorBalance<T> = Zero::zero();
				let mut own_oxor_balance: OxorBalance<T> = Zero::zero();
				let mut own_power = 0;
				...
				support
					...
					.for_each(|(nominator, power_u128)| {
						...
						let (origin_xor_balance, origin_oxor_balance) = Self::stake_of(&nominator);
						let xor_balance = if let Ok(xor_balance) =
							helpers_128bit::multiply_by_rational(
								origin_xor_balance.saturated_into(),
								power_u128,
								origin_power_u128,
							) {
							xor_balance.saturated_into()
						} else {
							log!(
								error,
								"[staking] Origin XOR: {:?}, Weight: {:?}, Origin Weight: {:?}",
								origin_xor_balance,
								power_u128,
								origin_power_u128
							);
						...
						let oxor_balance = if let Ok(oxor_balance) =
							helpers_128bit::multiply_by_rational(
								origin_oxor_balance.saturated_into(),
								power_u128,
								origin_power_u128,
							) {
							oxor_balance.saturated_into()
						} else {
							log!(
								error,
								"[staking] Origin OXOR: {:?}, Weight: {:?}, Origin Weight: {:?}",
								origin_oxor_balance,
								power_u128,
								origin_power_u128
							);
						...
						if nominator == validator {
							own_xor_balance = own_xor_balance.saturating_add(xor_balance);
							own_oxor_balance = own_oxor_balance.saturating_add(oxor_balance);
...
impl<T: Config> OnDepositRedeem<T::AccountId, XorBalance<T>> for Module<T> {
	fn on_deposit_redeem(
		...
		// The timestamp unit is different between Ethereum and Oracol
		...
		if let Some(controller) = Self::bonded(&stash) {
			...
			T::XorCurrency::transfer(&backing, &stash, amount, KeepAlive)?;
			...
			*active_xor = active_xor.saturating_add(amount);
			*active_deposit_xor = active_deposit_xor.saturating_add(amount);
			deposit_items.push(TimeDepositItem {
				value: amount,
				start_time,
				expire_time,
		...
		Self::deposit_event(RawEvent::BondXor(amount, start_time, expire_time));
...
/// To unify *XOR* and *OXOR* balances.
#[derive(PartialEq, Eq, Clone, Encode, Decode, RuntimeDebug)]
pub enum StakingBalance<XorBalance, OxorBalance>
where
	XorBalance: HasCompact,
	OxorBalance: HasCompact,
{
	XorBalance(XorBalance),
	OxorBalance(OxorBalance),
}
impl<XorBalance, OxorBalance> Default for StakingBalance<XorBalance, OxorBalance>
where
	XorBalance: Zero + HasCompact,
	OxorBalance: Zero + HasCompact,
{
	fn default() -> Self {
		StakingBalance::XorBalance(Zero::zero())
	}
}
...
pub struct StakingLedger<AccountId, XorBalance, OxorBalance, BlockNumber>
where
	XorBalance: HasCompact,
	OxorBalance: HasCompact,
{
	...
	/// The total amount of the stash's *XOR* that will be at stake in any forthcoming
	/// rounds.
	#[codec(compact)]
	...
	/// The total amount of the stash's *OXOR* that will be at stake in any forthcoming
	/// rounds.
	#[codec(compact)]
	pub active_oxor: OxorBalance,

	/// If you deposit *XOR* for a minimum period,
	/// you can get *OXOR* as bonus which can also be used for staking.
	pub deposit_items: Vec<TimeDepositItem<XorBalance>>,

	/// The staking lock on *XOR* balance, use for updating oracol balance module's lock
	pub xor_staking_lock: StakingLock<XorBalance, BlockNumber>,
	/// The staking lock on *OXOR* balance, use for updating oracol balance module's lock
...
impl<AccountId, XorBalance, OxorBalance, BlockNumber>
	StakingLedger<AccountId, XorBalance, OxorBalance, BlockNumber>
where
	XorBalance: Copy + AtLeast32BitUnsigned + Saturating,
	OxorBalance: Copy + AtLeast32BitUnsigned + Saturating,
	BlockNumber: Copy + PartialOrd,
	TsInMs: PartialOrd,
{
	pub fn xor_locked_amount_at(&self, at: BlockNumber) -> XorBalance {
		self.xor_staking_lock.locked_amount(at)
	}

	pub fn oxor_locked_amount_at(&self, at: BlockNumber) -> OxorBalance {
		self.oxor_staking_lock.locked_amount(at)
	}

	/// Re-bond funds that were scheduled for unlocking.
	fn rebond(&mut self, plan_to_rebond_xor: XorBalance, plan_to_rebond_oxor: OxorBalance) {
		...
		update(
			&mut self.active_xor,
			&mut self.xor_staking_lock,
			plan_to_rebond_xor,
		);
		update(
			&mut self.active_oxor,
			&mut self.oxor_staking_lock,
			plan_to_rebond_oxor,
		);
	}
	...
	pub fn slash(
		&mut self,
		slash_xor: XorBalance,
		slash_oxor: OxorBalance,
		bn: BlockNumber,
		ts: TsInMs,
	) -> (XorBalance, OxorBalance) {
		let slash_out_of = |active_xor: &mut XorBalance,
		                    active_deposit_xor: &mut XorBalance,
		                    deposit_item: &mut Vec<TimeDepositItem<XorBalance>>,
		                    active_oxor: &mut OxorBalance,
		                    slash_xor: &mut XorBalance,
		                    slash_oxor: &mut OxorBalance| {
			let slashable_active_xor = (*slash_xor).min(*active_xor);
			let slashable_active_oxor = (*slash_oxor).min(*active_oxor);

			if !slashable_active_xor.is_zero() {
				let slashable_normal_xor = *active_xor - *active_deposit_xor;
				if let Some(mut slashable_deposit_xor) =
					slashable_active_xor.checked_sub(&slashable_normal_xor)
				{
					*active_deposit_xor -= slashable_deposit_xor;

					deposit_item.drain_filter(|item| {
						if ts >= item.expire_time {
							true
						} else {
							if slashable_deposit_xor.is_zero() {
								false
							} else {
								if let Some(new_slashable_deposit_xor) =
									slashable_deposit_xor.checked_sub(&item.value)
								{
									slashable_deposit_xor = new_slashable_deposit_xor;
									true
								} else {
									item.value -= sp_std::mem::replace(
										&mut slashable_deposit_xor,
										Zero::zero(),
									);
									false
								}
				...
				*active_xor -= slashable_active_xor;
				*slash_xor -= slashable_active_xor;
			}

			if !slashable_active_oxor.is_zero() {
				*active_oxor -= slashable_active_oxor;
				*slash_oxor -= slashable_active_oxor;
			}
		};

		let (mut apply_slash_xor, mut apply_slash_oxor) = (slash_xor, slash_oxor);
		let StakingLedger {
			active_xor,
			active_deposit_xor,
			deposit_items,
		...
		slash_out_of(
			...
			active_oxor,
			&mut apply_slash_xor,
			&mut apply_slash_oxor,
		);

		if !apply_slash_xor.is_zero() {
			xor_staking_lock.unbondings.drain_filter(|lock| {
				if bn >= lock.until {
					true
				} else {
					if apply_slash_xor.is_zero() {
						...
						if apply_slash_xor >= lock.amount {
							apply_slash_xor -= lock.amount;
							true
						} else {
							lock.amount -=
								sp_std::mem::replace(&mut apply_slash_xor, Zero::zero());
							false
						}
					}
				}
		...
		if !apply_slash_oxor.is_zero() {
			oxor_staking_lock.unbondings.drain_filter(|lock| {
				if bn >= lock.until {
					true
				} else {
					if apply_slash_oxor.is_zero() {
						...
						if apply_slash_oxor > lock.amount {
							apply_slash_oxor -= lock.amount;

							true
						} else {
							lock.amount -=
								sp_std::mem::replace(&mut apply_slash_oxor, Zero::zero());
							false
						}
					}
				}
		...
		(slash_xor - apply_slash_xor, slash_oxor - apply_slash_oxor)
	}
}

/// The *XOR* under deposit.
#[derive(PartialEq, Eq, Clone, Encode, Decode, RuntimeDebug)]
pub struct TimeDepositItem<XorBalance: HasCompact> {
	#[codec(compact)]
	pub value: XorBalance,
	#[codec(compact)]
...
pub struct UnappliedSlash<AccountId, XorBalance, OxorBalance> {
	/// The stash ID of the offending validator.
	validator: AccountId,
	/// The validator's own slash.
	own: slashing::RK<XorBalance, OxorBalance>,
	/// All other slashed stakers and amounts.
	others: Vec<(AccountId, slashing::RK<XorBalance, OxorBalance>)>,
	/// Reporters of the offence; bounty payout recipients.
	reporters: Vec<AccountId>,
	/// The amount of payout.
	payout: slashing::RK<XorBalance, OxorBalance>,
}
