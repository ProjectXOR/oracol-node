...
pub(crate) type RKT<T> = RK<XorBalance<T>, OxorBalance<T>>;
...
pub(crate) struct SpanRecord<XorBalance, OxorBalance> {
	slashed: RK<XorBalance, OxorBalance>,
	paid_out: RK<XorBalance, OxorBalance>,
}

impl<XorBalance, OxorBalance> SpanRecord<XorBalance, OxorBalance> {
	/// The value of stash balance slashed in this span.
	#[cfg(test)]
	pub(crate) fn amount_slashed(&self) -> &RK<XorBalance, OxorBalance> {
		&self.slashed
	}
}
...
pub(crate) struct SlashParams<'a, T: 'a + Config> {
	...
	pub(crate) exposure: &'a Exposure<T::AccountId, XorBalance<T>, OxorBalance<T>>,
...
pub(crate) fn compute_slash<T: Config>(
...
) -> Option<UnappliedSlash<T::AccountId, XorBalance<T>, OxorBalance<T>>> {
	let SlashParams {
		stash,
		slash,
		exposure,
	...
	let own_slash = RK {
		r: slash * exposure.own_xor_balance,
		k: slash * exposure.own_oxor_balance,
	};
	...
	for nominator in &exposure.others {
		...
		let era_slash = {
			let own_slash_prior = RK {
				r: prior_slash_p * nominator.xor_balance,
				k: prior_slash_p * nominator.oxor_balance,
			};
			let own_slash_by_validator = RK {
				r: slash * nominator.xor_balance,
				k: slash * nominator.oxor_balance,
			};
...
pub fn do_slash<T: Config>(
	...
	slashed_xor: &mut XorNegativeImbalance<T>,
	slashed_oxor: &mut OxorNegativeImbalance<T>,
) {
	...
	let (slash_xor, slash_oxor) = ledger.slash(
		value.r,
		value.k,
	...
	if !slash_xor.is_zero() {
		slashed = true;

		let (imbalance, missing) = T::XorCurrency::slash(stash, slash_xor);

		slashed_xor.subsume(imbalance);
	...
	if !slash_oxor.is_zero() {
		slashed = true;

		let (imbalance, missing) = T::OxorCurrency::slash(stash, slash_oxor);

		slashed_oxor.subsume(imbalance);
...
pub(crate) fn apply_slash<T: Config>(
	unapplied_slash: UnappliedSlash<T::AccountId, XorBalance<T>, OxorBalance<T>>,
) {
	let mut slashed_xor = <XorNegativeImbalance<T>>::zero();
	let mut slashed_oxor = <OxorNegativeImbalance<T>>::zero();
...
fn pay_reporters<T: Config>(
	reward_payout: RKT<T>,
	slashed_xor: XorNegativeImbalance<T>,
	slashed_oxor: OxorNegativeImbalance<T>,
	reporters: &[T::AccountId],
) {
	if reporters.is_empty() || reward_payout.is_zero() {
		T::XorSlash::on_unbalanced(slashed_xor);
		T::OxorSlash::on_unbalanced(slashed_oxor);

		return;
	}

	// take rewards out of the slashed imbalance.
	let xor_reward_payout = reward_payout.r.min(slashed_xor.peek());
	let (mut xor_reward_payout, mut xor_slashed) = slashed_xor.split(xor_reward_payout);
	let oxor_reward_payout = reward_payout.k.min(slashed_oxor.peek());
	let (mut oxor_reward_payout, mut oxor_slashed) = slashed_oxor.split(oxor_reward_payout);

	let xor_per_reporter = xor_reward_payout.peek() / (reporters.len() as u32).into();
	let oxor_per_reporter = oxor_reward_payout.peek() / (reporters.len() as u32).into();

	for reporter in reporters {
		if !xor_per_reporter.is_zero() {
			let (xor_reporter_reward, xor_rest) = xor_reward_payout.split(xor_per_reporter);
			xor_reward_payout = xor_rest;

			// this cancels out the reporter reward imbalance internally, leading
			// to no change in total issuance.
			T::XorCurrency::resolve_creating(reporter, xor_reporter_reward);
		}

		if !oxor_per_reporter.is_zero() {
			let (oxor_reporter_reward, oxor_rest) = oxor_reward_payout.split(oxor_per_reporter);
			oxor_reward_payout = oxor_rest;

			// this cancels out the reporter reward imbalance internally, leading
			// to no change in total issuance.
			T::OxorCurrency::resolve_creating(reporter, oxor_reporter_reward);
		}
	}

	// the rest goes to the on-slash imbalance handler (e.g. treasury)
	xor_slashed.subsume(xor_reward_payout); // remainder of reward division remains.
	T::XorSlash::on_unbalanced(xor_slashed);

	// the rest goes to the on-slash imbalance handler (e.g. treasury)
	oxor_slashed.subsume(oxor_reward_payout); // remainder of reward division remains.
	T::OxorSlash::on_unbalanced(oxor_slashed);
}
