...
// --- oracol ---
use crate::{self as oracol_staking, *};
...
parameter_types! {
	...
	pub static XorRewardRemainderUnbalanced: Balance = 0;
}
impl Config for Test {
	...
	type XorRewardRemainder = XorRewardRemainderMock;
	type XorSlash = ();
	type XorReward = ();
...
frame_support::construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		Staking: oracol_staking::{Pallet, Call, Storage, Config<T>, Event<T>},
...
impl ExtBuilder {
	...
	pub fn init_oxor(mut self, init: bool) -> Self {
		self.init_oxor = init;
		self
	}
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		if self.init_oxor {
			let _ = oracol_balances::GenesisConfig::<Test, OxorInstance> {
				balances: vec![
					(1, 10 * balance_factor),
					(2, 20 * balance_factor),
					(3, 300 * balance_factor),
		...
		let _ = oracol_staking::GenesisConfig::<Test> {
			history_depth: 84,
			stakers,
...
pub struct XorRewardRemainderMock;
impl OnUnbalanced<XorNegativeImbalance<Test>> for XorRewardRemainderMock {
	fn on_nonzero_unbalanced(amount: XorNegativeImbalance<Test>) {
		XOR_REWARD_REMAINDER_UNBALANCED.with(|v| {
...
pub fn assert_ledger_consistent(controller: AccountId) {
	...
	assert_eq!(ledger.active_xor, ledger.xor_staking_lock.staking_amount);
	assert_eq!(ledger.active_oxor, ledger.oxor_staking_lock.staking_amount);

	let real_total_xor: Balance = ledger
		.xor_staking_lock
		.unbondings
		.iter()
		.fold(ledger.active_xor, |a, c| a + c.amount);
	let real_total_oxor: Balance = ledger
		.oxor_staking_lock
		.unbondings
		.iter()
		.fold(ledger.active_oxor, |a, c| a + c.amount);

	assert!(
		ledger.active_xor >= Xor::minimum_balance()
			|| ledger.active_oxor >= Oxor::minimum_balance()
			|| (ledger.active_xor == 0 && ledger.active_oxor == 0),
		"{}: active ledger amount ({}/{}) must be greater than ED {}/{}",
		controller,
		ledger.active_xor,
		ledger.active_oxor,
		Xor::minimum_balance(),
		Oxor::minimum_balance()
	);
}
...
fn bond(stash: AccountId, controller: AccountId, val: StakingBalanceT<Test>) {
	match val {
		StakingBalance::XorBalance(r) => {
			let _ = Xor::make_free_balance_be(&(stash), r);
			let _ = Xor::make_free_balance_be(&(controller), r);
		}
		StakingBalance::OxorBalance(k) => {
			let _ = Oxor::make_free_balance_be(&(stash), k);
			let _ = Oxor::make_free_balance_be(&(controller), k);
		}
	}
...
pub(crate) fn current_total_payout_for_duration(duration: TsInMs) -> Balance {
	inflation::compute_total_payout::<Test>(
		duration,
		Staking::living_time(),
		<Test as Config>::Cap::get() - Xor::total_issuance(),
...
pub(crate) fn staking_events() -> Vec<oracol_staking::Event<Test>> {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| {
			if let Event::oracol_staking(inner) = e {
				Some(inner)
			} else {
				None
			}
...
pub(crate) fn xor_balances(who: &AccountId) -> (Balance, Balance) {
	(Xor::free_balance(who), Xor::reserved_balance(who))
}
pub(crate) fn oxor_balances(who: &AccountId) -> (Balance, Balance) {
	(Oxor::free_balance(who), Oxor::reserved_balance(who))
}

pub(crate) fn xor_power(stake: Balance) -> Power {
	Staking::currency_to_power(stake, Staking::xor_pool())
}
