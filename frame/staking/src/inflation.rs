...
pub fn compute_total_payout<T: Config>(
	era_duration: TsInMs,
	living_time: TsInMs,
	total_left: XorBalance<T>,
	payout_fraction: Perbill,
) -> (XorBalance<T>, XorBalance<T>) {
	log::info!(
		target: "oracol-staking",
	...
	(
		<XorBalance<T>>::saturated_from::<Balance>(payout),
		<XorBalance<T>>::saturated_from::<Balance>(inflation),
	)
}
...
pub fn compute_oxor_reward<T: Config>(value: XorBalance<T>, months: u8) -> OxorBalance<T> {
