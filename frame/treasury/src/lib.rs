...
mod types {
	...
	pub type XorBalance<T, I> = <XorCurrency<T, I> as Currency<AccountId<T>>>::Balance;
	pub type XorPositiveImbalance<T, I> =
		<XorCurrency<T, I> as Currency<AccountId<T>>>::PositiveImbalance;
	pub type XorNegativeImbalance<T, I> =
		<XorCurrency<T, I> as Currency<AccountId<T>>>::NegativeImbalance;
	pub type OxorBalance<T, I> = <OxorCurrency<T, I> as Currency<AccountId<T>>>::Balance;
	pub type OxorPositiveImbalance<T, I> =
		<OxorCurrency<T, I> as Currency<AccountId<T>>>::PositiveImbalance;
	pub type OxorNegativeImbalance<T, I> =
		<OxorCurrency<T, I> as Currency<AccountId<T>>>::NegativeImbalance;

	type AccountId<T> = <T as frame_system::Config>::AccountId;
	type XorCurrency<T, I> = <T as Config<I>>::XorCurrency;
	type OxorCurrency<T, I> = <T as Config<I>>::OxorCurrency;
}
...
pub trait Config<I = DefaultInstance>: frame_system::Config {
	...
	type TipReportDepositBase: Get<XorBalance<Self, I>>;

	/// The amount held on deposit per byte within the tip report reason or bounty description.
	type DataDepositPerByte: Get<XorBalance<Self, I>>;

	/// The overarching event type.
	type Event: From<Event<Self, I>> + Into<<Self as frame_system::Config>::Event>;

	/// Handler for the unbalanced decrease when slashing for a rejected proposal or bounty.
	type OnSlashXor: OnUnbalanced<XorNegativeImbalance<Self, I>>;
	/// Handler for the unbalanced decrease when slashing for a rejected proposal or bounty.
	type OnSlashOxor: OnUnbalancedOxor<OxorNegativeImbalance<Self, I>>;

	/// Fraction of a proposal's value that should be bonded in order to place the proposal.
	/// An accepted proposal gets these back. A rejected proposal does not.
	type ProposalBond: Get<Permill>;

	/// Minimum amount of *XOR* that should be placed in a deposit for making a proposal.
	type XorProposalBondMinimum: Get<XorBalance<Self, I>>;
	/// Minimum amount of *OXOR* that should be placed in a deposit for making a proposal.
	type OxorProposalBondMinimum: Get<OxorBalance<Self, I>>;
	...
	type BountyDepositBase: Get<XorBalance<Self, I>>;
	...
	type BountyValueMinimum: Get<XorBalance<Self, I>>;

	/// Maximum acceptable reason length.
	type MaximumReasonLength: Get<u32>;

	/// Handler for the unbalanced decrease when treasury funds are burned.
	type XorBurnDestination: OnUnbalanced<XorNegativeImbalance<Self, I>>;

	/// Handler for the unbalanced decrease when treasury funds are burned.
	type OxorBurnDestination: OnUnbalancedOxor<OxorNegativeImbalance<Self, I>>;
...
decl_storage! {
	trait Store for Module<T: Config<I>, I: Instance = DefaultInstance> as OracolTreasury {
		...
		Proposals
			...
			=> Option<TreasuryProposal<T::AccountId, XorBalance<T, I>, OxorBalance<T, I>>>;
		...
		pub Tips
			...
			=> Option<OpenTip<T::AccountId, XorBalance<T, I>, T::BlockNumber, T::Hash>>;
		...
		pub Bounties
			...
			=> Option<Bounty<T::AccountId, XorBalance<T, I>, T::BlockNumber>>;
	...
	add_extra_genesis {
		build(|_config| {
			...
			{
				let min = T::XorCurrency::minimum_balance();

				if T::XorCurrency::free_balance(&account_id) < min {
			...
			{
				let min = T::OxorCurrency::minimum_balance();

				if T::OxorCurrency::free_balance(&account_id) < min {
...
decl_event!(
	...
	where
		...
		XorBalance = XorBalance<T, I>,
		OxorBalance = OxorBalance<T, I>,
	{
		/// New proposal. [proposal_index]
		Proposed(ProposalIndex),
		/// We have ended a spend period and will now allocate funds. [budget_remaining_xor]
		Spending(XorBalance, OxorBalance),
		/// Some funds have been allocated. [proposal_index, award, beneficiary]
		Awarded(ProposalIndex, XorBalance, OxorBalance, AccountId),
		/// A proposal was rejected; funds were slashed. [proposal_index, slashed]
		Rejected(ProposalIndex, XorBalance, OxorBalance),
		/// Some of our funds have been burnt. [burn]
		Burnt(XorBalance, OxorBalance),
		/// Spending has finished; this is the amount that rolls over until next spend. [budget_remaining_xor]
		Rollover(XorBalance, OxorBalance),
		/// Some *XOR* have been deposited. [deposit]
		DepositXor(XorBalance),
		/// Some *OXOR* have been deposited. [deposit]
		DepositOxor(OxorBalance),
		/// A new tip suggestion has been opened. [tip_hash]
		NewTip(Hash),
		/// A tip suggestion has reached threshold and is closing. [tip_hash]
		TipClosing(Hash),
		/// A tip suggestion has been closed. [tip_hash, who, payout]
		TipClosed(Hash, AccountId, XorBalance),
		/// A tip suggestion has been retracted. [tip_hash]
		TipRetracted(Hash),
		/// New bounty proposal. [index]
		BountyProposed(BountyIndex),
		/// A bounty proposal was rejected; funds were slashed. [index, bond]
		BountyRejected(BountyIndex, XorBalance),
		/// A bounty proposal is funded and became active. [index]
		BountyBecameActive(BountyIndex),
		/// A bounty is awarded to a beneficiary. [index, beneficiary]
		BountyAwarded(BountyIndex, AccountId),
		/// A bounty is claimed by beneficiary. [index, payout, beneficiary]
		BountyClaimed(BountyIndex, XorBalance, AccountId),
...
decl_module! {
		...
		origin: T::Origin
	{
		...
		/// Minimum amount of *XOR* that should be placed in a deposit for making a proposal.
		const XorProposalBondMinimum: XorBalance<T, I> = T::XorProposalBondMinimum::get();
		/// Minimum amount of *OXOR* that should be placed in a deposit for making a proposal.
		const OxorProposalBondMinimum: OxorBalance<T, I> = T::OxorProposalBondMinimum::get();
		...
		/// The amount held on deposit for placing a tip report.
		const TipReportDepositBase: XorBalance<T, I> = T::TipReportDepositBase::get();

		/// The amount held on deposit per byte within the tip report reason or bounty description.
		const DataDepositPerByte: XorBalance<T, I> = T::DataDepositPerByte::get();

		/// The treasury's module id, used for deriving its sovereign account ID.
		const ModuleId: ModuleId = T::ModuleId::get();

		/// The amount held on deposit for placing a bounty proposal.
		const BountyDepositBase: XorBalance<T, I> = T::BountyDepositBase::get();
		...
		const BountyValueMinimum: XorBalance<T, I> = T::BountyValueMinimum::get();
		...
		fn propose_spend(
			origin,
			#[compact] xor_value: XorBalance<T, I>,
			#[compact] oxor_value: OxorBalance<T, I>,
			beneficiary: <T::Lookup as StaticLookup>::Source
		) {
			let proposer = ensure_signed(origin)?;
			let beneficiary = T::Lookup::lookup(beneficiary)?;

			let xor_bond = Self::calculate_bond::<_, T::XorProposalBondMinimum>(xor_value);
			let oxor_bond = Self::calculate_bond::<_, T::OxorProposalBondMinimum>(oxor_value);

			T::XorCurrency::reserve(&proposer, xor_bond)
				.map_err(|_| <Error<T, I>>::InsufficientProposersBalance)?;
			T::OxorCurrency::reserve(&proposer, oxor_bond)
				.map_err(|_| <Error<T, I>>::InsufficientProposersBalance)?;
		...
		fn reject_proposal(origin, #[compact] proposal_id: ProposalIndex) {
			...
			let xor_bond = proposal.xor_bond;
			let imbalance_xor = T::XorCurrency::slash_reserved(&proposal.proposer, xor_bond).0;
			T::OnSlashXor::on_unbalanced(imbalance_xor);

			let oxor_bond = proposal.oxor_bond;
			let imbalance_oxor = T::OxorCurrency::slash_reserved(&proposal.proposer, oxor_bond).0;
			T::OnSlashOxor::on_unbalanced(imbalance_oxor);

			Self::deposit_event(<Event<T, I>>::Rejected(proposal_id, xor_bond, oxor_bond));
		}
		...
		fn report_awesome(origin, reason: Vec<u8>, who: T::AccountId) {
			...
			T::XorCurrency::reserve(&finder, deposit)?;
		...
		fn retract_tip(origin, hash: T::Hash) {
			...
			if !tip.deposit.is_zero() {
				let err_amount = T::XorCurrency::unreserve(&who, tip.deposit);
		...
		fn tip_new(origin, reason: Vec<u8>, who: T::AccountId, #[compact] tip_value: XorBalance<T, I>) {
		...
		fn tip(origin, hash: T::Hash, #[compact] tip_value: XorBalance<T, I>) {
			let tipper = ensure_signed(origin)?;
			ensure!(T::Tippers::contains(&tipper), BadOrigin);
		...
		fn unassign_curator(
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResult {
				...
				let slash_curator = |curator: &T::AccountId, curator_deposit: &mut XorBalance<T, I>| {
					let imbalance = T::XorCurrency::slash_reserved(curator, *curator_deposit).0;
					T::OnSlashXor::on_unbalanced(imbalance);
						...
						match maybe_sender {
							...
							Some(sender) => {
								...
								if sender != *curator {
									...
									let err_amount = T::XorCurrency::unreserve(&curator, bounty.curator_deposit);
									debug_assert!(err_amount.is_zero());
									// Continue to change bounty status below...
								}
							},
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResult {
				...
				match bounty.status {
					BountyStatus::CuratorProposed { ref curator } => {
						...
						T::XorCurrency::reserve(curator, deposit)?;
		...
		fn claim_bounty(origin, #[compact] bounty_id: BountyIndex) {
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResult {
				...
				if let BountyStatus::PendingPayout { curator, beneficiary, unlock_at } = bounty.status {
					...
					let balance = T::XorCurrency::free_balance(&bounty_account);
					let fee = bounty.fee.min(balance); // just to be safe
					let payout = balance.saturating_sub(fee);
					let err_amount = T::XorCurrency::unreserve(&curator, bounty.curator_deposit);
					debug_assert!(err_amount.is_zero());
					let res = T::XorCurrency::transfer(&bounty_account, &curator, fee, AllowDeath); // should not fail
					debug_assert!(res.is_ok());
					let res = T::XorCurrency::transfer(&bounty_account, &beneficiary, payout, AllowDeath); // should not fail
					debug_assert!(res.is_ok());
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResultWithPostInfo {
				...
				match &bounty.status {
					BountyStatus::Proposed => {
						...
						let imbalance = T::XorCurrency::slash_reserved(&bounty.proposer, value).0;
						T::OnSlashXor::on_unbalanced(imbalance);
						*maybe_bounty = None;
				...
				let balance = T::XorCurrency::free_balance(&bounty_account);
				let res = T::XorCurrency::transfer(&bounty_account, &Self::account_id(), balance, AllowDeath); // should not fail
				debug_assert!(res.is_ok());
				*maybe_bounty = None;
...
impl<T: Config<I>, I: Instance> Module<T, I> {
	...
	fn create_bounty(
		...
		T::XorCurrency::reserve(&proposer, bond)
			.map_err(|_| <Error<T, I>>::InsufficientProposersBalance)?;
	...
	fn insert_tip_and_check_closing(
		tip: &mut OpenTip<T::AccountId, XorBalance<T, I>, T::BlockNumber, T::Hash>,
		tipper: T::AccountId,
		tip_value: XorBalance<T, I>,
	) -> bool {
	...
	fn retain_active_tips(tips: &mut Vec<(T::AccountId, XorBalance<T, I>)>) {
	...
	fn payout_tip(
		hash: T::Hash,
		tip: OpenTip<T::AccountId, XorBalance<T, I>, T::BlockNumber, T::Hash>,
	) {
		let mut tips = tip.tips;
		Self::retain_active_tips(&mut tips);
		tips.sort_by_key(|i| i.1);
		let treasury = Self::account_id();
		let max_payout = Self::pot::<T::XorCurrency>();
		let mut payout = tips[tips.len() / 2].1.min(max_payout);
		if !tip.deposit.is_zero() {
			let err_amount = T::XorCurrency::unreserve(&tip.finder, tip.deposit);
			debug_assert!(err_amount.is_zero());
		}
		if tip.finders_fee {
			if tip.finder != tip.who {
				...
				let res = T::XorCurrency::transfer(&treasury, &tip.finder, finders_fee, KeepAlive);
				debug_assert!(res.is_ok());
			}
		}
		// same as above: best-effort only.
		let res = T::XorCurrency::transfer(&treasury, &tip.who, payout, KeepAlive);
		debug_assert!(res.is_ok());
	...
	fn spend_funds() -> Weight {
		...
		let mut budget_remaining_xor = Self::pot::<T::XorCurrency>();
		let mut budget_remaining_oxor = Self::pot::<T::OxorCurrency>();

		Self::deposit_event(RawEvent::Spending(
			budget_remaining_xor,
			budget_remaining_oxor,
		));

		let mut missed_any_xor = false;
		let mut imbalance_xor = <XorPositiveImbalance<T, I>>::zero();

		let mut missed_any_oxor = false;
		let mut imbalance_oxor = <OxorPositiveImbalance<T, I>>::zero();
		...
		let proposals_len = <Approvals<I>>::mutate(|v| {
			...
			v.retain(|&index| {
				...
				if let Some(p) = Self::proposals(index) {
					if p.xor_value > budget_remaining_xor || p.oxor_value > budget_remaining_oxor
					{
						if p.xor_value > budget_remaining_xor {
							missed_any_xor = true;
						}

						if p.oxor_value > budget_remaining_oxor {
							missed_any_oxor = true;
						}
					...
					if p.xor_value <= budget_remaining_xor {
						budget_remaining_xor -= p.xor_value;

						// return their deposit.
						let err_amount = T::XorCurrency::unreserve(&p.proposer, p.xor_bond);
						debug_assert!(err_amount.is_zero());

						// provide the allocation.
						imbalance_xor.subsume(T::XorCurrency::deposit_creating(
							&p.beneficiary,
							p.xor_value,
						));
					}
					if p.oxor_value <= budget_remaining_oxor {
						budget_remaining_oxor -= p.oxor_value;

						// return their deposit.
						let err_amount = T::OxorCurrency::unreserve(&p.proposer, p.oxor_bond);
						debug_assert!(err_amount.is_zero());

						// provide the allocation.
						imbalance_oxor.subsume(T::OxorCurrency::deposit_creating(
							&p.beneficiary,
							p.oxor_value,
						));
					}
			...
			v.retain(|&index| {
				<Bounties<T, I>>::mutate(index, |bounty| {
					...
					if let Some(bounty) = bounty {
						if bounty.value <= budget_remaining_xor {
							budget_remaining_xor -= bounty.value;

							bounty.status = BountyStatus::Funded;

							// return their deposit.
							let err_amount =
								T::XorCurrency::unreserve(&bounty.proposer, bounty.bond);
							debug_assert!(err_amount.is_zero());
		...
		{
			let burn_xor = if !missed_any_xor {
				// burn some proportion of the remaining budget if we run a surplus.
				let burn = (T::Burn::get() * budget_remaining_xor).min(budget_remaining_xor);
				budget_remaining_xor -= burn;

				let (debit, credit) = T::XorCurrency::pair(burn);
				imbalance_xor.subsume(debit);
				T::XorBurnDestination::on_unbalanced(credit);

				burn
			} else {
				Zero::zero()
			};
			let burn_oxor = if !missed_any_oxor {
				let burn = (T::Burn::get() * budget_remaining_oxor).min(budget_remaining_oxor);
				budget_remaining_oxor -= burn;

				let (debit, credit) = T::OxorCurrency::pair(burn);
				imbalance_oxor.subsume(debit);
				T::OxorBurnDestination::on_unbalanced(credit);
			...
			Self::deposit_event(RawEvent::Burnt(burn_xor, burn_oxor));
		}

		// Must never be an error, but better to be safe.
		// proof: budget_remaining_xor is account free balance minus ED;
		// Thus we can't spend more than account free balance minus ED;
		// Thus account is kept alive; qed;
		if let Err(problem) = T::XorCurrency::settle(
			&Self::account_id(),
			imbalance_xor,
		...
		if let Err(problem) = T::OxorCurrency::settle(
			&Self::account_id(),
			imbalance_oxor,
			WithdrawReasons::TRANSFER,
			KeepAlive,
...
impl<T: Config<I>, I: Instance> OnUnbalanced<XorNegativeImbalance<T, I>> for Module<T, I> {
	fn on_nonzero_unbalanced(amount: XorNegativeImbalance<T, I>) {
		let numeric_amount = amount.peek();

		// Must resolve into existing but better to be safe.
		let _ = T::XorCurrency::resolve_creating(&Self::account_id(), amount);

		Self::deposit_event(RawEvent::DepositXor(numeric_amount));
	}
}
// FIXME: Ugly hack due to https://github.com/rust-lang/rust/issues/31844#issuecomment-557918823
impl<T: Config<I>, I: Instance> OnUnbalancedOxor<OxorNegativeImbalance<T, I>> for Module<T, I> {
	fn on_nonzero_unbalanced(amount: OxorNegativeImbalance<T, I>) {
		let numeric_amount = amount.peek();

		// Must resolve into existing but better to be safe.
		let _ = T::OxorCurrency::resolve_creating(&Self::account_id(), amount);

		Self::deposit_event(RawEvent::DepositOxor(numeric_amount));
	}
}
...
#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug)]
pub struct TreasuryProposal<AccountId, XorBalance, OxorBalance> {
	/// The account proposing it.
	proposer: AccountId,
	/// The account to whom the payment should be made if the proposal is accepted.
	beneficiary: AccountId,
	/// The (total) *XOR* that should be paid if the proposal is accepted.
	xor_value: XorBalance,
	/// The (total) *OXOR* that should be paid if the proposal is accepted.
	oxor_value: OxorBalance,
	/// The *XOR* held on deposit (reserved) for making this proposal.
	xor_bond: XorBalance,
	/// The *OXOR* held on deposit (reserved) for making this proposal.
	oxor_bond: OxorBalance,
}
