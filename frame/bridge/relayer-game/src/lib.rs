...
pub trait Config<I: Instance = DefaultInstance>: frame_system::Config {
	...
	/// Handler for the unbalanced *XOR* reduction when slashing a relayer.
	type XorSlash: OnUnbalanced<XorNegativeImbalance<Self, I>>;

	/// A regulator to adjust relay args for a specific chain
	type RelayerGameAdjustor: AdjustableRelayerGame<
		Balance = XorBalance<Self, I>,
...
decl_error! {
	pub enum Error for Module<T: Config<I>, I: Instance> {
		...
		RelayAffirmationDup,
		/// Usable *XOR* for Stake - INSUFFICIENT
		StakeIns,
...
decl_storage! {
	trait Store for Module<T: Config<I>, I: Instance = DefaultInstance> as OracolRelayerGame {
...
impl<T: Config<I>, I: Instance> Module<T, I> {
	...
	) -> Result<XorBalance<T, I>, DispatchError> {
		let stake = T::RelayerGameAdjustor::estimate_stake(round, affirmations_count);

		ensure!(
			T::XorCurrency::usable_balance(relayer) >= stake,
			<Error<T, I>>::StakeIns
		);
	...
	where
		F: FnOnce(XorBalance<T, I>) -> XorBalance<T, I>,
	{
		let stakes = calc_stakes(Self::stakes_of(relayer));

		if stakes.is_zero() {
			T::XorCurrency::remove_lock(T::LockId::get(), relayer);
	...
	pub fn slash_on(relayer: &AccountId<T>, stake: XorBalance<T, I>) {
		Self::update_stakes_with(relayer, |old_stakes| old_stakes.saturating_sub(stake));

		T::XorSlash::on_unbalanced(T::XorCurrency::slash(relayer, stake).0);
	}

	pub fn payout_honesties_and_slash_evils(
		honesties: BTreeMap<AccountId<T>, (XorBalance<T, I>, XorBalance<T, I>)>,
		evils: BTreeMap<AccountId<T>, XorBalance<T, I>>,
	) {
		for (relayer, (unstakes, rewards)) in honesties {
			...
			// Reward honesty
			T::XorCurrency::deposit_creating(&relayer, rewards);
		}
		...
		for (relayer, slashs) in evils {
			...
			// Punish evil
			T::XorCurrency::slash(&relayer, slashs);
		}
	}
	...
	pub fn settle_with_challenge(
		...
		let mut honesties = <BTreeMap<AccountId<T>, (XorBalance<T, I>, XorBalance<T, I>)>>::new();
		// BTreeMap<(relayer, slash)>
		let mut evils = <BTreeMap<AccountId<T>, XorBalance<T, I>>>::new();
		...
		if let Some(last_round_relay_affirmations) = relay_affirmations.last() {
			...
			match last_round_winning_relay_chain_indexes.len() {
				...
				1 => {
					...
					// BTreeMap<(relayer, unstake, reward)>
					let mut honesties =
						<BTreeMap<AccountId<T>, (XorBalance<T, I>, XorBalance<T, I>)>>::new();
					// BTreeMap<(relayer, slash)>
					let mut evils = <BTreeMap<AccountId<T>, XorBalance<T, I>>>::new();
