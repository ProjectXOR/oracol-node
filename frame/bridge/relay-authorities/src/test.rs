...
fn insufficient_stake_should_fail() {
	new_test_ext().execute_with(|| {
		...
		let _ = Xor::deposit_creating(&123, 1);
		...
		let _ = Xor::deposit_creating(&123, 11);
...
fn add_authorities_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert!(Xor::locks(1).is_empty());
		assert!(Xor::locks(2).is_empty());
		assert!(Xor::locks(3).is_empty());
...
fn kill_candidates_should_work() {
	new_test_ext().execute_with(|| {
		...
		for i in 1..=max_candidates {
			...
			assert!(!Xor::locks(i as AccountId).is_empty());
		}
...
fn mmr_root_signed_event_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			vec![Event::oracol_relay_authorities(RawEvent::MMRRootSigned(
				10,
				DEFAULT_MMR_ROOT,
...
fn authorities_change_signed_event_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			relay_authorities_events(),
			vec![Event::oracol_relay_authorities(
...
fn slash_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Xor::total_balance(&9), 900);
		assert_eq!(Xor::total_balance(&1), 100);
		assert_eq!(Xor::total_balance(&2), 200);
		...
		assert_eq!(
			relay_authorities_events(),
			vec![
				Event::oracol_relay_authorities(RawEvent::SlashOnMisbehavior(9, 1)),
				Event::oracol_relay_authorities(RawEvent::SlashOnMisbehavior(1, 50)),
				Event::oracol_relay_authorities(RawEvent::SlashOnMisbehavior(2, 60)),
			]
		);
		assert!(Xor::locks(9).is_empty());
		assert!(Xor::locks(1).is_empty());
		assert!(Xor::locks(2).is_empty());
		assert_eq!(Xor::total_balance(&9), 899);
		assert_eq!(Xor::total_balance(&1), 50);
		assert_eq!(Xor::total_balance(&2), 140);
		...
		for i in 2..10 {
			...
			assert_eq!(
				relay_authorities_events(),
				vec![
					Event::oracol_relay_authorities(RawEvent::SlashOnMisbehavior(9, 0)),
					Event::oracol_relay_authorities(RawEvent::SlashOnMisbehavior(1, 0)),
					Event::oracol_relay_authorities(RawEvent::SlashOnMisbehavior(2, 0)),
				]
			);
			assert!(Xor::locks(9).is_empty());
			assert!(Xor::locks(1).is_empty());
