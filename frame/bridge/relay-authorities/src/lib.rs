...
mod types {
	...
	pub type XorCurrency<T, I> = <T as Config<I>>::XorCurrency;
	...
	pub type RelayAuthorityT<T, I> =
		RelayAuthority<AccountId<T>, RelayAuthoritySigner<T, I>, XorBalance<T, I>, BlockNumber<T>>;
...
pub trait Config<I: Instance = DefaultInstance>: frame_system::Config {
	...
	type OracolMMR: MMR<Self::BlockNumber, Self::Hash>;
...
decl_event! {
		...
		RelayAuthoritySignature = RelayAuthoritySignature<T, I>,
	{
		...
		SlashOnMisbehavior(AccountId, XorBalance),
	}
}
...
decl_storage! {
	trait Store for Module<T: Config<I>, I: Instance = DefaultInstance> as OracolRelayAuthorities {
	...
	add_extra_genesis {
		config(authorities): Vec<(AccountId<T>, RelayAuthoritySigner<T, I>, XorBalance<T, I>)>;
		build(|config| {
		...
		origin: T::Origin
	{
		...
		pub fn request_authority(
			...
			if let Some(scheduled_authorities_change) = <NextAuthorities<T, I>>::get() {
				...
				<XorCurrency<T, I>>::usable_balance(&account_id) > stake,
				<Error<T, I>>::StakeIns
			);
			...
			<Candidates<T, I>>::try_mutate(|candidates| {
				...
				if candidates.len() == T::MaxCandidates::get() {
					...
					<XorCurrency<T, I>>::remove_lock(T::LockId::get(), &weep_out.account_id);
				}

				<XorCurrency<T, I>>::set_lock(
					T::LockId::get(),
					&account_id,
					LockFor::Common { amount: stake },
					WithdrawReasons::all()
		...
		pub fn cancel_request(origin) {
			...
			let _ = Self::remove_candidate_by_id_with(
				&account_id,
				|| <XorCurrency<T, I>>::remove_lock(T::LockId::get(), &account_id)
			);
		}
		...
		pub fn kill_candidates(origin) {
			...
			for RelayAuthority { account_id, .. } in <Candidates<T, I>>::take() {
				<XorCurrency<T, I>>::remove_lock(lock_id, &account_id);
			}
		}
		...
		pub fn submit_signed_mmr_root(
			...
			let mmr_root =
				T::OracolMMR::get_root(block_number).ok_or(<Error<T, I>>::OracolMMRRootNRY)?;
					...
					} in authorities.iter_mut()
					{
							...
							.is_none()
						{
							...
							if !stake.is_zero() {
								...
								T::XorCurrency::remove_lock(T::LockId::get(), account_id);
								<XorCurrency<T, I>>::slash(account_id, *stake);
