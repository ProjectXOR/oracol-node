...
mod types {
	...
	pub type XorBalance<T> = <CurrencyT<T> as Currency<AccountId<T>>>::Balance;
...
use oracol_support::{balance::*, traits::EthereumReceipt as EthereumReceiptT};
...
#[cfg(feature = "std")]
oracol_support::impl_genesis! {
	struct DagsMerkleRootsLoader {
		dags_merkle_roots: Vec<H128>
	}
}
decl_storage! {
	trait Store for Module<T: Config> as OracolEthereumRelay {
		...
		/// The highest ethereum block number that record in oracol
...
decl_module! {
		...
		origin: T::Origin
	{
		...
		/// receipt, we need to provide this as part of proof, because in Oracol Relay, we only have
		...
		/// # </weight>
		#[weight = 10_000_000]
		pub fn set_receipt_verify_fee(origin, #[compact] new: XorBalance<T>) {
...
impl<T: Config> EthereumReceiptT<AccountId<T>, XorBalance<T>> for Module<T> {
	...
	fn receipt_verify_fee() -> XorBalance<T> {
