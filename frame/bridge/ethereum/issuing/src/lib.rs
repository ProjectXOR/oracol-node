...
use sp_std::vec::Vec;
// --- oracol ---
use oracol_ethereum_issuing_contract::{
	Abi, Event as EthEvent, Log as EthLog, TokenBurnInfo, TokenRegisterInfo,
};
use oracol_evm::{GasWeightMapping, IssuingHandler};
use oracol_relay_primitives::relay_authorities::*;
use oracol_support::{balance::*, traits::EthereumReceipt};
...
decl_storage! {
	trait Store for Module<T: Config> as OracolEthereumIssuing {
...
decl_module! {
		...
		origin: T::Origin
	{
		...
		#[weight = <T as oracol_evm::Config>::GasWeightMapping::gas_to_weight(0x100000)]
