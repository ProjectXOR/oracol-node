...
//! This is implemented in Oracol's fork of substrate: https://github.com/oracol-network/substrate
//! The Pull request link is https://github.com/oracol-network/substrate/pull/1
//!
//! ## Implementation
//! We are using the MMR library from https://github.com/nervosnetwork/merkle-mountain-range
//! Pull request: https://github.com/oracol-network/oracol/pull/358
//!
//! ## References
//! Oracol Relay's Technical Paper:
//! https://github.com/oracol-network/rfcs/blob/master/paper/Oracol_Relay_Sublinear_Optimistic_Relay_for_Interoperable_Blockchains_v0.7.pdf
//!
...
// --- oracol ---
use oracol_header_mmr_rpc_runtime_api::{Proof, RuntimeDispatchInfo};
use oracol_relay_primitives::MMR as MMRT;
use oracol_support::impl_rpc;
...
decl_storage! {
	trait Store for Module<T: Config> as OracolHeaderMMR {
		...
		fn on_finalize(_block_number: T::BlockNumber) {
			...
			if let Ok(parent_mmr_root) = mmr.get_root() {
				if mmr.commit().is_ok() {
					...
					log::error!("[oracol-header-mmr] FAILED to Commit MMR");
				}
			} else {
				log::error!("[oracol-header-mmr] FAILED to Calculate MMR");
			}
		}
	}
}
