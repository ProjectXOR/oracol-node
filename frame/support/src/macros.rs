...
macro_rules! impl_account_data {
	(
		...
		use oracol_support::balance::BalanceInfo;
		...
		)?
		{
			...
			pub free_oxor: Balance,
			pub reserved_oxor: Balance,
			$(pub $fname: $ftype),*
		}

		impl BalanceInfo<$btype, $xor_instance> for AccountData<$btype> {
			fn free(&self) -> $btype {
				self.free
			}
			...
			fn usable(
				&self,
				reasons: oracol_support::balance::LockReasons,
				frozen_balance: oracol_support::balance::FrozenBalance<$btype>,
			) -> $btype {
				self.free.saturating_sub(frozen_balance.frozen_for(reasons))
			}
		}

		impl BalanceInfo<$btype, $oxor_instance> for AccountData<$btype> {
			fn free(&self) -> $btype {
				self.free_oxor
			}
			fn set_free(&mut self, new_free_oxor: $btype) {
				self.free_oxor = new_free_oxor;
			}

			fn reserved(&self) -> $btype {
				self.reserved_oxor
			}
			fn set_reserved(&mut self, new_reserved_oxor: $btype) {
				self.reserved_oxor = new_reserved_oxor;
			}

			fn total(&self) -> $btype {
				self.free_oxor.saturating_add(self.reserved_oxor)
			}
			...
			fn usable(
				...
				self.free_oxor.saturating_sub(frozen_balance.frozen_for(reasons))
			}
		}
	};
}
