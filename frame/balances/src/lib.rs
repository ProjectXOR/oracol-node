...
//! - [`LockableCurrency`](oracol_support::traits::LockableCurrency): Functions for
...
pub mod pallet {
	...
	use oracol_balances_rpc_runtime_api::RuntimeDispatchInfo;
	use oracol_support::{balance::*, impl_rpc, traits::BalanceInfo};
...
pub mod migration {
	...
	pub mod try_runtime {
		...
		generate_storage_types!("Instance0OracolBalances", "TotalIssuance", OldXorTotalIssuance => Value<()>);
		generate_storage_types!("Balances", "TotalIssuance", NewXorTotalIssuance => Value<()>);
		generate_storage_types!("Instance1OracolBalances", "TotalIssuance", OldOxorTotalIssuance => Value<()>);
		generate_storage_types!("Oxor", "TotalIssuance", NewOxorTotalIssuance => Value<()>);

		pub fn pre_migrate() -> Result<(), &'static str> {
			assert!(OldXorTotalIssuance::exists());
			assert!(!NewXorTotalIssuance::exists());
			assert!(OldOxorTotalIssuance::exists());
			assert!(!NewOxorTotalIssuance::exists());

			log::info!("Migrating `Instance0Oracol` to `Balances`...");
			migration::migrate(b"Instance0OracolBalances", b"Balances");

			log::info!("Migrating `Instance1Oracol` to `Oxor`...");
			migration::migrate(b"Instance1OracolBalances", b"Oxor");
	...
	pub fn migrate(old_pallet_name: &[u8], new_pallet_name: &[u8]) {
		// Balances: Instance0OracolBalances
		// Oxor: Instance1OracolBalances
