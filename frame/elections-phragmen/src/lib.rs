...
decl_storage! {
	trait Store for Module<T: Config> as OracolPhragmenElection {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn vote(
			...
			T::Currency::set_lock(
				T::ModuleId::get(),
				&who,
				LockFor::Common { amount: locked_stake },
				WithdrawReasons::all(),
			);
...
mod tests {
	...
	frame_support::construct_runtime! {
			...
			UncheckedExtrinsic = UncheckedExtrinsic
		{
			...
			Balances: oracol_balances::<Instance0>::{Pallet, Call, Event<T>, Config<T>},
	...
	impl ExtBuilder {
		...
		pub fn build_and_execute(self, test: impl FnOnce() -> ()) {
			...
			let mut ext: sp_io::TestExternalities = GenesisConfig {
				oracol_balances_Instance0: oracol_balances::GenesisConfig::<Test, XorInstance> {
					balances: vec![
