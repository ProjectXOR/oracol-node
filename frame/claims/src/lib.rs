...
pub trait Config: frame_system::Config {
	...
	/// The *XOR* currency.
	type XorCurrency: LockableCurrency<Self::AccountId>;
...
decl_event!(
	...
	{
		/// Someone claimed some *XOR*s. [account, address, amount]
		Claimed(AccountId, AddressT, XorBalance),
	}
);
...
decl_storage! {
	trait Store for Module<T: Config> as OracolClaims {
		ClaimsFromEth
			get(fn claims_from_eth)
			: map hasher(identity) AddressT => Option<XorBalance<T>>;
		ClaimsFromOldoxor
			get(fn claims_from_oldoxor)
			: map hasher(identity) AddressT => Option<XorBalance<T>>;
	}
	add_extra_genesis {
		...
		build(|config| {
			...
			let mut total = <XorBalance<T>>::zero();

			if dot.is_empty() && eth.is_empty() && oldoxor.is_empty() {
				log::error!("[oracol-claims] Genesis Claims List is Set to EMPTY");
			} else {
				// Eth Address
				for Account { address, backed_xor } in dot {
					// DOT:XOR = 1:50
					let backed_xor = (*backed_xor).saturated_into();
					<ClaimsFromEth<T>>::insert(address.0, backed_xor);
					total += backed_xor;
				}
				for Account { address, backed_xor } in eth {
					let backed_xor = (*backed_xor).saturated_into();
					<ClaimsFromEth<T>>::insert(address.0, backed_xor);
					total += backed_xor;
				}
				...
				// Oldoxor Address
				for Account { address, backed_xor } in oldoxor {
					let backed_xor = (*backed_xor).saturated_into();
					<ClaimsFromOldoxor<T>>::insert(address.0, backed_xor);
					total += backed_xor;
				}
			}

			let minimum_balance = T::XorCurrency::minimum_balance();
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
			...
			match signature {
				OtherSignature::Eth(signature) => {
					...
					let balance_due = <ClaimsFromEth<T>>::get(&signer)
						...
						Self::pot::<T::XorCurrency>() >= balance_due,
				...
				OtherSignature::Oldoxor(signature) => {
					let signer = Self::oldoxor_recover(&signature, &data)
						.ok_or(<Error<T>>::InvalidSignature)?;
					let balance_due = <ClaimsFromOldoxor<T>>::get(&signer)
						.ok_or(<Error<T>>::SignerHasNoClaim)?;
					...
					<ClaimsFromOldoxor<T>>::remove(&signer);
		...
		fn mint_claim(origin, who: OtherAddress, value: XorBalance<T>) {
			ensure_root(origin)?;

			match who {
				OtherAddress::Eth(who) => {
					T::XorCurrency::deposit_creating(&Self::account_id(), value);
					<ClaimsFromEth<T>>::insert(who, value);
				}
				OtherAddress::Oldoxor(who) => {
					T::XorCurrency::deposit_creating(&Self::account_id(), value);
					<ClaimsFromOldoxor<T>>::insert(who, value);
				}
			}
		}
		...
		fn move_claim(origin,
			...
			match old {
				...
				OtherAddress::Oldoxor(old) => if let OtherAddress::Oldoxor(new) = new {
					ensure!(
						!<ClaimsFromOldoxor<T>>::contains_key(&new),
						<Error<T>>::MoveToExistedAddress
					);

					<ClaimsFromOldoxor<T>>::take(&old).map(|c| <ClaimsFromOldoxor<T>>::insert(&new, c));
				} else {
					Err(<Error<T>>::NewAddressTypeMis)?;
				}
			}
...
impl<T: Config> Module<T> {
	...
	// Constructs the message that RPC's `personal_sign` and `sign` would sign.
	// Oldoxor have different signing specs: https://github.com/oldoxorprotocol/tips/issues/104
	fn oldoxor_signable_message(what: &[u8], signed_message: &[u8]) -> Vec<u8> {
		let prefix = T::Prefix::get();
		let mut l = 32;
		let mut rev = Vec::new();
		while l > 0 {
	...
	// Attempts to recover the Oldoxor address from a message signature signed by using
	// the Oldoxor RPC's `personal_sign` and `oldoxor_sign`.
	fn oldoxor_recover(s: &EcdsaSignature, what: &[u8]) -> Option<AddressT> {
		let msg = keccak_256(&Self::oldoxor_signable_message(
			what,
			b"\x19OLDoxor Signed Message:\n",
		));
		let mut res = AddressT::default();
		res.copy_from_slice(&keccak_256(&secp256k1_ecdsa_recover(&s.0, &msg).ok()?[..])[12..]);
		Some(res)
		...
		match call {
			Call::claim(account, signature) => {
				...
				match signature {
					...
					OtherSignature::Oldoxor(signature) => {
						let maybe_signer = Self::oldoxor_recover(&signature, &data);
						let signer = if let Some(s) = maybe_signer {
							s
						} else {
							return InvalidTransaction::Custom(
						...
						if !<ClaimsFromOldoxor<T>>::contains_key(&signer) {
							return Err(InvalidTransaction::Custom(
								ValidityError::SignerHasNoClaim as _,
							)
							.into());
...
mod secp_utils {
	...
	pub fn oldoxor_sig<T: Config>(
		secret: &secp256k1::SecretKey,
		what: &[u8],
		signed_message: &[u8],
	) -> EcdsaSignature {
		let msg = keccak_256(&<super::Pallet<T>>::oldoxor_signable_message(
			&to_ascii_hex(what)[..],
			signed_message,
		));
		let (sig, recovery_id) = secp256k1::sign(&secp256k1::Message::parse(&msg), secret);
...
mod tests {
	...
	// --- oracol ---
	use crate::{self as oracol_claims, secp_utils::*, *};
	...
	const OLDoxor_SIGNED_MESSAGE: &'static [u8] = b"\x19OLDoxor Signed Message:\n";
	...
	frame_support::construct_runtime! {
			...
			UncheckedExtrinsic = UncheckedExtrinsic
		{
			...
			Xor: oracol_balances::<Instance0>::{Pallet, Call, Storage, Config<T>},
			Claims: oracol_claims::{Pallet, Call, Storage, Config}
		}
	}
	...
	fn new_test_ext() -> sp_io::TestExternalities {
		...
		<oracol_balances::GenesisConfig<Test, XorInstance>>::default()
			.assimilate_storage(&mut t)
			.unwrap();
		oracol_claims::GenesisConfig {
			claims_list: ClaimsList {
				...
				oldoxor: vec![Account {
					address: OldoxorAddress(addr(&carol())),
					backed_xor: 300,
				}],
			},
		}
	...
	fn basic_setup_works() {
		new_test_ext().execute_with(|| {
			assert_eq!(Xor::usable_balance(&Claims::account_id()), 600);

			assert_eq!(Claims::claims_from_eth(&addr(&alice())), Some(100));
			assert_eq!(Claims::claims_from_oldoxor(&addr(&alice())), None);

			assert_eq!(Claims::claims_from_eth(&addr(&bob())), Some(200));
			assert_eq!(Claims::claims_from_oldoxor(&addr(&bob())), None);

			assert_eq!(Claims::claims_from_eth(&addr(&carol())), None);
			assert_eq!(Claims::claims_from_oldoxor(&addr(&carol())), Some(300));
		});
	}
	...
	fn serde_works() {
		...
		let x = OldoxorAddress(array_bytes::hex2array_unchecked!(
			"0x0123456789abcdef0123456789abcdef01234567",
			20
		));
		let y = serde_json::to_string(&x).unwrap();
		assert_eq!(y, "\"410123456789abcdef0123456789abcdef01234567\"");
		let z: OldoxorAddress = serde_json::from_str(&y).unwrap();
		assert_eq!(x.0, z.0);
	}
	...
	fn claiming_works() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(Xor::usable_balance(&Claims::account_id()), 500);
			...
			assert_eq!(Xor::free_balance(&2), 200);
			assert_eq!(Xor::usable_balance(&Claims::account_id()), 300);

			assert_eq!(Xor::free_balance(3), 0);
			assert_ok!(Claims::claim(
				Origin::none(),
				3,
				OtherSignature::Oldoxor(oldoxor_sig::<Test>(
					&carol(),
					&3u64.encode(),
					OLDoxor_SIGNED_MESSAGE
				)),
			));
			assert_eq!(Xor::free_balance(&3), 300);
			assert_eq!(Xor::usable_balance(&Claims::account_id()), 0);
		});
	}
		...
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Claims::move_claim(
					Origin::signed(6),
					OtherAddress::Eth(addr(&alice())),
					OtherAddress::Oldoxor(addr(&carol())),
				),
			...
			assert_eq!(Xor::free_balance(&42), 100);
			assert_eq!(
				Xor::usable_balance(&Claims::account_id()),
				total_claims() - 100
			);
		});
	}
	...
	fn add_claim_works() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(Xor::usable_balance(&Claims::account_id()), 800);
			assert_ok!(Claims::claim(
				Origin::none(),
				69,
				OtherSignature::Eth(eth_sig::<Test>(
			...
			assert_eq!(Xor::free_balance(&69), 200);
			assert_eq!(Xor::usable_balance(&Claims::account_id()), 600);
		});
	}
	...
	fn real_oldoxor_sig_works() {
		new_test_ext().execute_with(|| {
			...
			let signer = Claims::oldoxor_recover(&sig, &who).unwrap();
	...
	fn validate_unsigned_works() {
		...
		new_test_ext().execute_with(|| {
			...
			assert_eq!(
				Claims::validate_unsigned(
					source,
					&oracol_claims::Call::claim(0, OtherSignature::Eth(EcdsaSignature([0; 65])))
				),
