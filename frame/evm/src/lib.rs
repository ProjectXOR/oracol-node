	...
	add_extra_genesis {
		...
		build(|config: &GenesisConfig| {
			for (address, account) in &config.accounts {
				T::XorAccountBasic::mutate_account_basic(&address, Account {
					balance: account.balance,
					nonce: account.nonce,
				});
				T::OxorAccountBasic::mutate_account_basic(&address, Account {
					balance: account.balance,
					nonce: account.nonce,
				});
...
impl<T: Config> Module<T> {
	...
	pub fn is_account_empty(address: &H160) -> bool {
		let account = T::XorAccountBasic::account_basic(address);
	...
	pub fn withdraw_fee(address: &H160, value: U256) {
		...
		T::XorAccountBasic::mutate_account_basic(
			&address,
			Account {
				nonce: account.nonce,
