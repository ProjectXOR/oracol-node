...
impl<T: Config> AccountBasic for RawAccountBasic<T> {
	...
	fn account_basic(address: &H160) -> Account {
		...
		let balance = T::XorCurrency::free_balance(&account_id);
	...
	fn mutate_account_basic(address: &H160, new: Account) {
		let account_id = T::AddressMapping::into_account_id(*address);
		let current = T::XorAccountBasic::account_basic(address);
		...
		if current.balance > new.balance {
			...
			T::XorCurrency::slash(&account_id, diff.low_u128().unique_saturated_into());
		} else if current.balance < new.balance {
			let diff = new.balance - current.balance;
			T::XorCurrency::deposit_creating(&account_id, diff.low_u128().unique_saturated_into());
		}
	}
...
impl Config for Test {
	...
	type XorAccountBasic = RawAccountBasic<Test>;
	type OxorAccountBasic = RawAccountBasic<Test>;
}
...
frame_support::construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		EVM: oracol_evm::{Pallet, Call, Storage, Config, Event<T>},
	}
}
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	<oracol_balances::GenesisConfig<Test, XorInstance>>::default()
		...
		.unwrap();
	<oracol_balances::GenesisConfig<Test, OxorInstance>>::default()
		.assimilate_storage(&mut t)
		.unwrap();
	oracol_evm::GenesisConfig { accounts }
