...
use codec::Decode;
use oracol_evm::{AddressMapping, Config};
...
/// WithDraw Precompile Contract, used to withdraw balance from evm account to oracol account
///
...
impl<T: Config> Precompile for WithDraw<T> {
	...
	fn execute(
		...
		let result = T::XorCurrency::transfer(
			&contract_address,
			&input.dest,
			context_value,
