...
use crate::{
	account_basic::{RemainBalanceOp, XorRemainBalance},
	Call, *,
};
use codec::Decode;
use oracol_evm::{Account, AddressMapping};
...
macro_rules! assert_balance {
	($evm_address:expr, $balance:expr, $left:expr, $right:expr) => {
		let account_id =
			<Test as oracol_evm::Config>::AddressMapping::into_account_id($evm_address);
		assert_eq!(
			<Test as oracol_evm::Config>::XorAccountBasic::account_basic(&$evm_address).balance,
			$balance
		);
		assert_eq!(
			<Test as oracol_evm::Config>::XorCurrency::free_balance(&account_id),
			$left
		);
		assert_eq!(
			<XorRemainBalance as RemainBalanceOp<Test, u64>>::remaining_balance(&account_id),
			$right
		);
	};
}
...
fn transaction_should_increment_nonce() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(
			<Test as oracol_evm::Config>::XorAccountBasic::account_basic(&alice.address).nonce,
			U256::from(1)
		);
	});
}
...
fn withdraw_with_enough_balance() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(
			<Test as oracol_evm::Config>::XorAccountBasic::account_basic(&alice.address).balance,
		...
		assert_eq!(
			<Test as Config>::XorCurrency::free_balance(dest),
			30000000000
		);
	});
}
...
fn withdraw_without_enough_balance_should_fail() {
	...
	ext.execute_with(|| {
		...
		let dest =
			<Test as frame_system::Config>::AccountId::decode(&mut &input_bytes[..]).unwrap();
		assert_eq!(<Test as Config>::XorCurrency::free_balance(&dest), 0);
	});
}
#[test]
...
fn mutate_account_works_well() {
	...
	ext.execute_with(|| {
		...
		<Test as oracol_evm::Config>::XorAccountBasic::mutate_account_basic(
			&test_addr,
			Account {
				nonce: U256::from(0),
