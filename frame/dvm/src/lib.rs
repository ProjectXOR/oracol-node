...
use sp_std::prelude::*;
// --- oracol ---
use oracol_evm::{AccountBasic, FeeCalculator, GasWeightMapping, Runner};
use oracol_support::evm::INTERNAL_CALLER;
...
pub type XorCurrency<T> = <T as Config>::XorCurrency;
pub type OxorCurrency<T> = <T as Config>::OxorCurrency;
...
pub trait Config:
	frame_system::Config<Hash = H256> + pallet_timestamp::Config + oracol_evm::Config
{
...
decl_storage! {
	trait Store for Module<T: Config> as Ethereum {
		...
		RemainingXorBalance get(fn get_xor_remaining_balances): map hasher(blake2_128_concat) T::AccountId => XorBalance<T>;
		/// Remaining oxor balance for account
		RemainingOxorBalance get(fn get_oxor_remaining_balances): map hasher(blake2_128_concat) T::AccountId => OxorBalance<T>;
	}
	add_extra_genesis {
...
decl_module! {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		#[weight = <T as oracol_evm::Config>::GasWeightMapping::gas_to_weight(transaction.gas_limit.unique_saturated_into())]
...
impl<T: Config> frame_support::unsigned::ValidateUnsigned for Module<T> {
	...
	fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
		if let Call::transact(transaction) = call {
			...
			let account_data =
				<T as oracol_evm::Config>::XorAccountBasic::account_basic(&origin);
...
impl<T: Config> Module<T> {
	...
	pub fn internal_transact(target: H160, input: Vec<u8>) -> DispatchResultWithPostInfo {
		...
		let nonce =
			<T as oracol_evm::Config>::XorAccountBasic::account_basic(&INTERNAL_CALLER).nonce;
