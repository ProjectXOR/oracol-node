...
impl<T: Config> Module<T> {
	...
	pub fn matuxor_referenda_at(
		...
		Self::matuxor_referenda_at_inner(n, next..last)
	}

	fn matuxor_referenda_at_inner(
		n: T::BlockNumber,
	...
	fn begin_block(now: T::BlockNumber) -> Result<Weight, DispatchError> {
		...
		for (index, info) in Self::matuxor_referenda_at_inner(now, next..last).into_iter() {
