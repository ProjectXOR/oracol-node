...
// --- oracol ---
use crate::{self as oracol_democracy, *};
use oracol_balances::Error as BalancesError;
...
impl Filter<Call> for BaseFilter {
	fn filter(call: &Call) -> bool {
		!matches!(
			call,
			&Call::Balances(oracol_balances::Call::set_balance(..))
		)
	}
}
...
frame_support::construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		Balances: oracol_balances::<Instance0>::{Pallet, Call, Storage, Config<T>, Event<T>},
		Scheduler: pallet_scheduler::{Pallet, Call, Storage, Config, Event<T>},
		Democracy: oracol_democracy::{Pallet, Call, Storage, Config, Event<T>},
	}
}
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	.unwrap();
	oracol_democracy::GenesisConfig::default()
		.assimilate_storage(&mut t)
		.unwrap();
...
fn set_balance_proposal(value: u64) -> Vec<u8> {
	Call::Balances(oracol_balances::Call::set_balance(42, value, 0)).encode()
}
