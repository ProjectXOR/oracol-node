## Oracol Chain
Oracol chain is written in [Rust](https://www.rust-lang.org/). A basic familiarity with Rust tooling is required.

To learn more about Oracol chain, please refer to **[Documentation](https://docs.oracol.org/)**.

### Clone
To clone the repo with its submodules run:
```bash
git clone --recursive https://github.com/ProjectXOR/oracol-node-pos
```
...
```bash
./target/release/oracol-node --dev
```

Purge the development chain's state:

```bash
./target/release/oracol-node purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_LOG=debug RUST_BACKTRACE=1 ./target/release/oracol-node -lruntime=debug --dev
```
...
```
./target/release/oracol-node benchmark \
    --chain=dev \
    --steps=50 \
    --repeat=20 \
    --pallet=module_poc \
