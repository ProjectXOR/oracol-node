...
use oracol_primitives::{AccountPublic, Balance, Nonce};
use oracol_runtime::BABE_GENESIS_EPOCH_CONFIG;
...
pub struct Extensions {
	...
	pub fork_blocks: sc_client_api::ForkBlocks<oracol_primitives::Block>,
	/// Known bad block hashes.
	pub bad_blocks: sc_client_api::BadBlocks<oracol_primitives::Block>,
}
...
pub fn public_testnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		// Bootnodes
		vec!["/dns/bootnode-t1.oracolscan.com/tcp/30334/p2p/12D3KooWKmFtS7BFtkkKWrP5ZcCpPFokmST2JFXFSsVBNeW5SXWg".parse().unwrap()],
		// Telemetry
		TelemetryEndpoints::new(vec![(TELEMETRY_URL.into(), 0)]).ok(),
		// Protocol ID
		Some("oracol_testnet"),
...
pub fn live_testnet_config() -> Result<ChainSpec, String> {
	ChainSpec::from_json_bytes(&include_bytes!("../../assets/oracol-testnet.json")[..])
}
...
pub fn mainnet_config() -> Result<ChainSpec, String> {
	...
	Ok(ChainSpec::from_genesis(
		...
		// Bootnodes
		vec![
			"/dns/bootnode.oracolscan.com/tcp/30333/p2p/12D3KooWFHSc9cUcyNtavUkLg4VBAeBnYNgy713BnovUa9WNY5pp".parse().unwrap(),
			"/dns/bootnode.oracol.org/tcp/30333/p2p/12D3KooWAQqcXvcvt4eVEgogpDLAdGWgR5bY1drew44We6FfJAYq".parse().unwrap(),
			"/dns/bootnode.oracol-node-pos.com/tcp/30333/p2p/12D3KooWCT7rnUmEK7anTp7svwr4GTs6k3XXnSjmgTcNvdzWzgWU".parse().unwrap(),
		],
		// Telemetry
		TelemetryEndpoints::new(vec![(TELEMETRY_URL.into(), 0)]).ok(),
		// Protocol ID
...
fn testnet_genesis(
	...
	const INITIAL_BALANCE: u128 = 100_000_000 * XOR;
	const INITIAL_STAKING: u128 =   1_000_000 * XOR;
...
fn mainnet_genesis(
	...
	let balances = initial_authorities
		...
		.chain(endowed_accounts.iter().cloned().map(|x| (x.0.clone(), x.1 * XOR)))
		.chain(
			get_all_module_accounts()
				.iter()
				.map(|x| (x.clone(), existential_deposit)),
...
/// Token
pub fn oracol_properties() -> Properties {
	let mut p = Properties::new();
	p.insert("ss58format".into(), 42.into());
	p.insert("tokenDecimals".into(), 18.into());
	p.insert("tokenSymbol".into(), "XOR".into());
	p
}
