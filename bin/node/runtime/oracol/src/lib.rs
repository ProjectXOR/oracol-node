...
//! The Oracol Node Template runtime. This can be compiled with `#[no_std]`, ready for Wasm.
...
pub mod impls {
	...
	pub mod relay {
		// --- oracol ---
		use crate::*;
		use oracol_relay_primitives::relayer_game::*;
	...
	oracol_support::impl_account_data! {
	...
	pub struct ToAuthor;
	impl OnUnbalanced<XorNegativeImbalance> for ToAuthor {
		fn on_nonzero_unbalanced(amount: XorNegativeImbalance) {
			let numeric_amount = amount.peek();
			let author = Authorship::author();
			Xor::resolve_creating(&Authorship::author(), amount);
			System::deposit_event(<oracol_balances::Event<Runtime, XorInstance>>::Deposit(
				author,
				numeric_amount,
			));
		}
	...
	impl OnUnbalanced<XorNegativeImbalance> for DealWithFees {
		fn on_unbalanceds<B>(mut fees_then_tips: impl Iterator<Item = XorNegativeImbalance>) {
	...
	impl WeightToFeePolynomial for WeightToFee {
		...
		fn polynomial() -> WeightToFeeCoefficients<Self::Balance> {
			// in Oldxor, extrinsic base weight (smallest non-zero weight) is mapped to 100 MILLI:
			let p = 100 * MILLI;
...
pub mod wasm {
	...
	pub const WASM_BINARY: &[u8] = include_bytes!("../../../../wasm/oracol_runtime.compact.wasm");
	#[cfg(all(feature = "std", not(any(target_arch = "x86_64", target_arch = "x86"))))]
	pub const WASM_BINARY_BLOATY: &[u8] = include_bytes!("../../../../wasm/oracol_runtime.wasm");
...
pub use oracol_staking::StakerStatus;
...
// --- oracol ---
use oracol_balances_rpc_runtime_api::RuntimeDispatchInfo as BalancesRuntimeDispatchInfo;
use oracol_evm::{Account as EVMAccount, FeeCalculator, Runner};
use oracol_header_mmr_rpc_runtime_api::RuntimeDispatchInfo as HeaderMMRRuntimeDispatchInfo;
use oracol_staking_rpc_runtime_api::RuntimeDispatchInfo as StakingRuntimeDispatchInfo;
use xor_primitives::*;
...
pub type SignedExtra = (
	...
	oracol_ethereum_relay::CheckEthereumRelayHeaderParcel<Runtime>,
);
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("Oracol"),
	impl_name: create_runtime_str!("Oracol"),
	authoring_version: 1,
...
frame_support::construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Balances: oracol_balances::<Instance0>::{Pallet, Call, Storage, Config<T>, Event<T>} = 4,
		Oxor: oracol_balances::<Instance1>::{Pallet, Call, Storage, Config<T>, Event<T>} = 5,
		...
		Staking: oracol_staking::{Pallet, Call, Storage, Config<T>, Event<T>} = 9,
		...
		HeaderMMR: oracol_header_mmr::{Pallet, Call, Storage} = 16,

		// Governance stuff; uncallable initially.
		Democracy: oracol_democracy::{Pallet, Call, Storage, Config, Event<T>} = 17,
		...
		ElectionsPhragmen: oracol_elections_phragmen::{Pallet, Call, Storage, Config<T>, Event<T>} = 20,
		TechnicalMembership: pallet_membership::<Instance0>::{Pallet, Call, Storage, Config<T>, Event<T>} = 21,
		Treasury: oracol_treasury::{Pallet, Call, Storage, Event<T>} = 22,

		Sudo: pallet_sudo::{Pallet, Call, Storage, Config<T>, Event<T>} = 23,

		// Claims. Usable initially.
		Claims: oracol_claims::{Pallet, Call, Storage, Config, Event<T>, ValidateUnsigned} = 24,

		// Vesting. Usable initially, but removed once all vesting is finished.
		Vesting: oracol_vesting::{Pallet, Call, Storage, Event<T>, Config<T>} = 25,
		...
		OracolOldxorIssuing: oracol_oldxor_issuing::{Pallet, Call, Storage, Config, Event<T>} = 33,
		OracolOldxorBacking: oracol_oldxor_backing::{Pallet, Storage, Config<T>} = 34,

		EthereumRelay: oracol_ethereum_relay::{Pallet, Call, Storage, Config<T>, Event<T>} = 35,
		EthereumBacking: oracol_ethereum_backing::{Pallet, Call, Storage, Config<T>, Event<T>} = 36,
		EthereumIssuing: oracol_ethereum_issuing::{Pallet, Call, Storage, Config, Event<T>} = 42,
		EthereumRelayerGame: oracol_relayer_game::<Instance0>::{Pallet, Storage} = 37,
		EthereumRelayAuthorities: oracol_relay_authorities::<Instance0>::{Pallet, Call, Storage, Config<T>, Event<T>} = 38,

		OldoxorBacking: oracol_oldoxor_backing::{Pallet, Storage, Config<T>} = 39,

		EVM: oracol_evm::{Pallet, Call, Storage, Config, Event<T>} = 40,
	...
	Call: From<LocalCall>,
{
	fn create_transaction<C: frame_system::offchain::AppCrypto<Self::Public, Self::Signature>>(
		...
		let extra: SignedExtra = (
			...
			oracol_ethereum_relay::CheckEthereumRelayHeaderParcel::<Runtime>::new(),
		);
...
impl_runtime_apis! {
	...
	impl oracol_balances_rpc_runtime_api::BalancesApi<Block, AccountId, Balance> for Runtime {
		fn usable_balance(instance: u8, account: AccountId) -> BalancesRuntimeDispatchInfo<Balance> {
			match instance {
				0 => Xor::usable_balance_rpc(account),
				1 => Oxor::usable_balance_rpc(account),
				_ => Default::default()
			}
		}
	}

	impl oracol_header_mmr_rpc_runtime_api::HeaderMMRApi<Block, Hash> for Runtime {
		fn gen_proof(
	...
	impl oracol_staking_rpc_runtime_api::StakingApi<Block, AccountId, Power> for Runtime {
	...
	impl dvm_rpc_runtime_api::EthereumRuntimeRPCApi<Block> for Runtime {
		fn chain_id() -> u64 {
			<Runtime as oracol_evm::Config>::ChainId::get()
		}

		fn gas_price() -> U256 {
			<Runtime as oracol_evm::Config>::FeeCalculator::min_gas_price()
		}

		fn account_basic(address: H160) -> EVMAccount {
			use oracol_evm::AccountBasic;

			<Runtime as oracol_evm::Config>::XorAccountBasic::account_basic(&address)
		}

		fn account_code_at(address: H160) -> Vec<u8> {
			oracol_evm::Module::<Runtime>::account_codes(address)
		}
		...
		fn storage_at(address: H160, index: U256) -> H256 {
			...
			oracol_evm::Module::<Runtime>::account_storages(address, H256::from_slice(&tmp[..]))
		}
		...
		fn call(
			...
			nonce: Option<U256>,
			estimate: bool,
		) -> Result<oracol_evm::CallInfo, sp_runtime::DispatchError> {
			let config = if estimate {
				let mut config = <Runtime as oracol_evm::Config>::config().clone();
			...
			<Runtime as oracol_evm::Config>::Runner::call(
				from,
				to,
				data,
				value,
				...
				gas_price,
				nonce,
				config.as_ref().unwrap_or(<Runtime as oracol_evm::Config>::config()),
		...
		fn create(
			...
			nonce: Option<U256>,
			estimate: bool,
		) -> Result<oracol_evm::CreateInfo, sp_runtime::DispatchError> {
			let config = if estimate {
				let mut config = <Runtime as oracol_evm::Config>::config().clone();
				config.estimate = true;
				Some(config)
			...
			<Runtime as oracol_evm::Config>::Runner::create(
				from,
				data,
				value,
				gas_limit.low_u64(),
...
impl OnRuntimeUpgrade for CustomOnRuntimeUpgrade {
	...
	fn pre_upgrade() -> Result<(), &'static str> {
		oracol_balances::migration::try_runtime::pre_migrate()
	}

	fn on_runtime_upgrade() -> Weight {
		oracol_balances::migration::migrate(b"Instance0OracolBalances", b"Balances");
		oracol_balances::migration::migrate(b"Instance1OracolBalances", b"Oxor");
