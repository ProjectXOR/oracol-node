...
use xor_primitives::{AccountId, Balance, Hash, Nonce, OpaqueBlock as Block, Power};
...
native_executor_instance!(
	pub OracolExecutor,
	oracol_runtime::api::dispatch,
	oracol_runtime::native_version,
);

/// A set of APIs that oracol-like runtimes must implement.
pub trait RuntimeApiCollection:
	...
	+ oracol_balances_rpc_runtime_api::BalancesApi<Block, AccountId, Balance>
	+ oracol_header_mmr_rpc_runtime_api::HeaderMMRApi<Block, Hash>
	+ oracol_staking_rpc_runtime_api::StakingApi<Block, AccountId, Power>
...
/// ORACOL client abstraction, this super trait only pulls in functionality required for
/// ORACOL internal crates like ORACOL-collator.
pub trait ORACOLClient<Block, Backend, Runtime>:
	Sized
	+ Send
	+ Sync
...
impl<Block, Backend, Runtime, Client> ORACOLClient<Block, Backend, Runtime> for Client
where
	Backend: sc_client_api::Backend<Block>,
	Block: sp_runtime::traits::Block,
	Client: Sized
...
fn set_prometheus_registry(config: &mut Configuration) -> Result<(), ServiceError> {
	if let Some(PrometheusConfig { registry, .. }) = config.prometheus_config.as_mut() {
		*registry = Registry::new_custom(Some("ORACOL".into()), None)?;
	}
...
/// Create a new ORACOL service for a full node.
#[cfg(feature = "full-node")]
pub fn xor_new_full(
	...
	(
		TaskManager,
		Arc<impl ORACOLClient<Block, FullBackend, oracol_runtime::RuntimeApi>>,
		RpcHandlers,
	),
	ServiceError,
> {
	let (components, client, rpc_handlers) = new_full::<
		oracol_runtime::RuntimeApi,
		OracolExecutor,
...
/// Create a new ORACOL service for a light client.
pub fn xor_new_light(config: Configuration) -> Result<(TaskManager, RpcHandlers), ServiceError> {
	new_light::<oracol_runtime::RuntimeApi, OracolExecutor>(config)
}
